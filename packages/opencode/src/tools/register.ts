...
function readEvidenceFilesSimple(evidenceDir: string, _cwd: string): Array<{ taskId: string; type: string }> {
	const evidence: Array<{ taskId: string; type: string }> = [];
	try {
		const fs = require('node:fs');
		if (!require('node:fs').existsSync(evidenceDir) || !require('node:fs').statSync(evidenceDir).isDirectory()) {
			return evidence;
		}
			...
			try {
				...
				if (data.required_gates && Array.isArray(data.required_gates)) {
					for (const gate of data.required_gates) {
						evidence.push({ taskId: data.taskId || filename.replace('.json', ''), type: gate });
					}
				}
			} catch {
				// Skip invalid files
...
export const check_gate_status: SwarmTool = createSwarmTool({
	description: 'Check the status of gates for a task. Reads .swarm/evidence/{taskId}.json and returns which gates have passed.',
...
export const checkpoint: SwarmTool = createSwarmTool({
	description: 'Save, restore, list, or delete git checkpoints. Creates commits with labels for easy restoration.',
	args: {} as Record<string, unknown>,
	execute: async (args: Record<string, unknown>, directory: string) => {
		...
		switch (action) {
			case 'save':
				if (!label) {
					return JSON.stringify({ action: 'save', success: false, error: 'label is required for save action' }, null, 2);
				}
				return handleSave(label, directory);
			case 'restore':
				if (!label) {
					return JSON.stringify({ action: 'restore', success: false, error: 'label is required for restore action' }, null, 2);
				}
			...
			case 'delete':
				if (!label) {
					return JSON.stringify({ action: 'delete', success: false, error: 'label is required for delete action' }, null, 2);
				}
				return handleDelete(label, directory);
			default:
				return JSON.stringify({ action, success: false, error: `Unknown action: ${action}` }, null, 2);
		}
	},
});
...
export const complexity_hotspots: SwarmTool = createSwarmTool({
	description: 'Identify high-risk code hotspots by combining git churn frequency with cyclomatic complexity estimates.',
	args: {} as Record<string, unknown>,
	execute: async (args: Record<string, unknown>, directory: string) => {
		...
		const extensions = extensionsArg ? extensionsArg.split(',').map(e => e.trim()) : ['ts', 'tsx', 'js', 'jsx', 'py', 'rs', 'ps1'];
...
export const declare_scope: SwarmTool = createSwarmTool({
	description: 'Set the file scope for coder delegations. Must be called before delegating to coder to enable scope containment checking.',
	args: {} as Record<string, unknown>,
	execute: async (args: Record<string, unknown>, directory: string) => {
		...
		const result = await executeDeclareScope({ taskId, files, whitelist, working_directory }, directory);
		return JSON.stringify(result, null, 2);
	},
});
...
export const diff: SwarmTool = createSwarmTool({
	description: 'Analyze git diff between commits, showing file changes and contract changes.',
	args: {} as Record<string, unknown>,
	execute: async (args: Record<string, unknown>, directory: string) => {
...
export const evidence_check: SwarmTool = createSwarmTool({
	description: 'Check which tasks have all required evidence gates completed and identify gaps.',
	args: {} as Record<string, unknown>,
	execute: async (args: Record<string, unknown>, directory: string) => {
		...
		const requiredTypes = required_types ?? ['coder', 'reviewer', 'test_engineer'];
...
export const extract_code_blocks: SwarmTool = createSwarmTool({
	description: 'Extract code blocks from markdown content and save them to files.',
	args: {} as Record<string, unknown>,
	execute: async (args: Record<string, unknown>, directory: string) => {
...
export const gitingest: SwarmTool = createSwarmTool({
	description: 'Fetch a GitHub repository content via gitingest.com for analysis.',
	args: {} as Record<string, unknown>,
	execute: async (args: Record<string, unknown>, _directory: string) => {
		...
		const result = await fetchGitingest({ url, pattern, patternType, maxFileSize });
		return result;
	},
});
...
export const knowledge_query: SwarmTool = createSwarmTool({
	...
	execute: async (args: Record<string, unknown>, directory: string) => {
		...
		const category = validateCategoryInput(args.category as string) ?? undefined;
		...
		const filteredSwarm = filterSwarmEntries(swarmEntries as Parameters<typeof filterSwarmEntries>[0], filters);
		const filteredHive = filterHiveEntries(hiveEntries as Parameters<typeof filterHiveEntries>[0], filters);
		
		// Format entries
		const formattedSwarm = filteredSwarm.slice(0, limit).map(e => formatSwarmEntry(e as Parameters<typeof formatSwarmEntry>[0]));
		const formattedHive = filteredHive.slice(0, limit).map(e => formatHiveEntry(e as Parameters<typeof formatHiveEntry>[0]));
...
export const pkg_audit: SwarmTool = createSwarmTool({
	...
	args: {} as Record<string, unknown>,
	execute: async (args: Record<string, unknown>, directory: string) => {
		const ecosystem = (args.ecosystem as 'auto' | 'npm' | 'pip' | 'cargo' | 'go' | 'dotnet' | 'ruby' | 'dart') ?? 'auto';
...
export const pre_check_batch: SwarmTool = createSwarmTool({
	description: 'Run multiple verification tools in parallel: lint, secretscan, SAST scan, and quality budget.',
	args: {} as Record<string, unknown>,
	execute: async (args: Record<string, unknown>, directory: string) => {
		const files = args.files as string[] | undefined;
		const sast_threshold = (args.sast_threshold as 'low' | 'medium' | 'high' | 'critical') ?? 'medium';
		
		const result = await runPreCheckBatch({ files: files ?? [], directory, sast_threshold }, directory);
		return JSON.stringify(result, null, 2);
	},
});
...
export const retrieve_summary: SwarmTool = createSwarmTool({
	description: 'Retrieve the full content of a stored tool output summary by its ID.',
	args: {} as Record<string, unknown>,
	execute: async (args: Record<string, unknown>, directory: string) => {
...
export const save_plan: SwarmTool = createSwarmTool({
	description: 'Save a structured implementation plan to .swarm/plan.json and .swarm/plan.md.',
	args: {} as Record<string, unknown>,
	execute: async (args: Record<string, unknown>, directory: string) => {
		...
		const result = await executeSavePlan({ phases, swarm_id, title, working_directory }, directory);
		return JSON.stringify(result, null, 2);
	},
});
...
export const schema_drift: SwarmTool = createSwarmTool({
	description: 'Compare OpenAPI spec against actual route implementations to find drift.',
	args: {} as Record<string, unknown>,
	execute: async (args: Record<string, unknown>, directory: string) => {
...
export const secretscan: SwarmTool = createSwarmTool({
	description: 'Scan directory for potential secrets (API keys, tokens, passwords) using regex patterns and entropy heuristics.',
	args: {} as Record<string, unknown>,
	execute: async (args: Record<string, unknown>, directory: string) => {
...
export const test_runner: SwarmTool = createSwarmTool({
	...
	args: {} as Record<string, unknown>,
	execute: async (args: Record<string, unknown>, directory: string) => {
		const scope = (args.scope as 'all' | 'convention' | 'graph') ?? 'convention';
		...
		const result = await runTests(framework, scope, files ?? [], coverage, timeout_ms, directory);
		return JSON.stringify(result, null, 2);
	},
});
...
export const todo_extract: SwarmTool = createSwarmTool({
	...
	execute: async (args: Record<string, unknown>, directory: string) => {
		...
		const pathsStr = Array.isArray(paths) ? paths.join(',') : (paths ?? directory);
		const result = await executeTodoExtract({ paths: pathsStr, tags }, directory);
		return result;
	},
});
...
export const update_task_status: SwarmTool = createSwarmTool({
	description: 'Update the status of a specific task in the implementation plan.',
	args: {} as Record<string, unknown>,
	execute: async (args: Record<string, unknown>, directory: string) => {
		const status = args.status as 'pending' | 'in_progress' | 'completed' | 'blocked';
		const task_id = args.task_id as string;
		
		const result = await executeUpdateTaskStatus({ status, task_id }, directory);
		return JSON.stringify(result, null, 2);
	},
});
...
export const write_retro: SwarmTool = createSwarmTool({
	...
	execute: async (args: Record<string, unknown>, directory: string) => {
		...
		const task_complexity = args.task_complexity as 'trivial' | 'simple' | 'moderate' | 'complex';
		...
		const top_rejection_reasons = args.top_rejection_reasons as string[] | undefined;

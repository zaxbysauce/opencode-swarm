...
import { hasActiveTurboMode } from '@opencode-swarm/core';
...
BATCHING DETECTION — you are batching if your coder delegation contains ANY of:
    ...
    - Multiple TASK objectives ("TASK: Refactor the processor and update the Phrases like "also", "while you're at it", "additionally", config")
    - "as well"
...
- If \`.swarm/spec.md\` EXISTS:
  ...
  - STALE SPEC DETECTION: If the spec heading or feature description does NOT match the current work being planned (e.g., spec describes "user authentication" but user is asking to plan "payment integration"), treat the spec as potentially stale and offer three options:
    ...
    3. **Skip spec entirely** → proceed with planning below ignoring the existing spec
...
export function createArchitectAgent(
	...
	const TURBO_MODE_BANNER = turboMode
		? `## ⚠️ TURBO MODE ACTIVE

You are operating in TURBO MODE. This means:
- Faster iteration, less verbose output
- Skip non-essential context in agent delegations
- Focus on delivering working code over comprehensive documentation
- Stage B (agent review) is BYPASSED — you are solely responsible for code quality

Use your judgment to balance speed vs. thoroughness.
`
		: '';

	const BASE_PROMPT = ARCHITECT_PROMPT.replace(
		'{{TURBO_MODE_BANNER}}',
		TURBO_MODE_BANNER,
	);
	...
	return {
		name: 'architect',
		description:
			'Orchestrator of a multi-agent swarm. Thinks, delegates, and coordinates agent workflows.',
		config: {
			model,
			temperature: 0.2,
			prompt,

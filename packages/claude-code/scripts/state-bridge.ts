...
export interface TaskState {
    ...
    workflowState: 'idle' | 'coder_delegated' | 'pre_check_passed' | 'reviewer_run' | 'tests_run' | 'complete';
}
...
export class StateBridge {
    ...
    private cachedState: MinimalSwarmState | null = null;
    ...
    load(): MinimalSwarmState {
        ...
        // Cold load from disk
        const state = this.coldLoad(planPath, snapshotPath, planMtime, snapshotMtime);
        ...
        try {
            ...
            try {
                if (existsSync(snapshotPath)) {
                    existingSnapshot = JSON.parse(readFileSync(snapshotPath, 'utf-8')) as Record<string, unknown>;
                }
            } catch { /* start fresh */ }
            
            // Merge updated delegation chains and tool counts
            existingSnapshot.delegationChains = Object.fromEntries(state.delegationChains);
            existingSnapshot.toolAggregates = Object.fromEntries(
                Array.from(state.toolCallCounts.entries()).map(([k, v]) => [k, { totalCalls: v }]),
            );
            
            // Write back to session/state.json
            mkdirSync(path.dirname(snapshotPath), { recursive: true });
            writeFileSync(snapshotPath, JSON.stringify(existingSnapshot, null, 2), 'utf-8');
    ...
    private tryLoadCache(planMtime: number, snapshotMtime: number): MinimalSwarmState | null {
        try {
            ...
            if (cache.planMtime !== planMtime || cache.snapshotMtime !== snapshotMtime) {
                return null;
            }
    ...
    private coldLoad(
        ...
        try {
            if (existsSync(snapshotPath)) {
                ...
                for (const [sessionId, chain] of Object.entries(snapshot.delegationChains ?? {})) {
                    state.delegationChains.set(sessionId, chain);
                }

                // Restore tool call counts
                for (const [tool, agg] of Object.entries(snapshot.toolAggregates ?? {})) {
    ...
    private statusToWorkflowState(status: string): TaskState['workflowState'] {
        switch (status) {
            ...
            case 'in_progress': return 'coder_delegated';
            default: return 'idle';
        }
    }

    private persistCache(state: MinimalSwarmState, planMtime: number, snapshotMtime: number): void {
        try {

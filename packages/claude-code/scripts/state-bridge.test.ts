...
import { StateBridge, createStateBridge, type MinimalSwarmState, type TaskState } from './state-bridge';
...
describe('StateBridge', () => {
    ...
    test('load() reads task states from plan.json', () => {
        ...
        writeFileSync(path.join(swarmDir, 'plan.json'), JSON.stringify(plan), 'utf-8');
    ...
    test('load() reads delegation chains from session/state.json', () => {
        ...
        const snapshot = {
            delegationChains: {
                ...
                'session-2': [
                    { from: 'user', to: 'planner', timestamp: 3000 },
                ],
            },
            toolAggregates: {
                'Read': { totalCalls: 50 },
        ...
        expect(chain1?.[1]).toEqual({ from: 'coder', to: 'reviewer', timestamp: 2000 });
    ...
    test('load() uses cache when mtimes match (cache hit)', () => {
        ...
        state1.taskStates.set('1.1', { id: '1.1', status: 'completed', workflowState: 'complete' });
    ...
    test('load() does cold load when mtimes differ (cache miss)', () => {
        ...
        writeFileSync(path.join(swarmDir, 'plan.json'), JSON.stringify(plan1), 'utf-8');
        writeFileSync(
            path.join(swarmDir, 'session', 'state.json'),
            JSON.stringify(snapshot),
            'utf-8'
        ...
        writeFileSync(path.join(swarmDir, 'plan.json'), JSON.stringify(plan2), 'utf-8');
    ...
    test('advanceTaskWorkflowState() advances forward correctly', () => {
        ...
        expect(state.taskStates.get('task-1')?.workflowState).toBe('coder_delegated');
        ...
        expect(state.taskStates.get('task-1')?.workflowState).toBe('pre_check_passed');
    ...
    test('advanceTaskWorkflowState() ignores backward transitions', () => {
        ...
        const state: MinimalSwarmState = {
            taskStates: new Map([
                ['task-1', { id: 'task-1', status: 'in_progress', workflowState: 'pre_check_passed' }],
            ]),
            delegationChains: new Map(),
            toolCallCounts: new Map(),
            sessionId: 'test-session',
    ...
    test('load() handles empty plan.json gracefully', () => {
        ...
        // Write empty plan
        writeFileSync(path.join(swarmDir, 'plan.json'), JSON.stringify({ phases: [] }), 'utf-8');

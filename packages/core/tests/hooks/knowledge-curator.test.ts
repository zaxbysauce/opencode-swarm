...
const mockAppendRejectedLesson = vi.fn();
...
const mockResolveSwarmRejectedPath = vi.fn();
const mockComputeConfidence = vi.fn();
const mockInferTags = vi.fn();

// Create local mock variables for utils
const mockReadSwarmFileAsync = vi.fn();
const mockSafeHook = vi.fn();
const mockValidateSwarmPath = vi.fn();
...
vi.mock('../../src/hooks/knowledge-validator.js', () => ({
	validateLesson: (...args: unknown[]) =>
		mockValidateLesson(...(args as Parameters<typeof mockValidateLesson>)),
	quarantineEntry: (...args: unknown[]) =>
		mockQuarantineEntry(...(args as Parameters<typeof mockQuarantineEntry>)),
}));

vi.mock('../../src/hooks/knowledge-reader.js', () => ({
	updateRetrievalOutcome: (...args: unknown[]) =>
		mockUpdateRetrievalOutcome(...(args as Parameters<typeof mockUpdateRetrievalOutcome>)),
}));

vi.mock('../../src/hooks/knowledge-store.js', () => ({
	resolveSwarmKnowledgePath: (...args: unknown[]) => mockResolveSwarmKnowledgePath(...(args as Parameters<typeof mockResolveSwarmKnowledgePath>)),
	resolveSwarmRejectedPath: (...args: unknown[]) => mockResolveSwarmRejectedPath(...(args as Parameters<typeof mockResolveSwarmRejectedPath>)),
	readKnowledge: (...args: unknown[]) => mockReadKnowledge(...(args as Parameters<typeof mockReadKnowledge>)),
	appendKnowledge: (...args: unknown[]) => mockAppendKnowledge(...(args as Parameters<typeof mockAppendKnowledge>)),
	appendRejectedLesson: (...args: unknown[]) => mockAppendRejectedLesson(...(args as Parameters<typeof mockAppendRejectedLesson>)),
	findNearDuplicate: (...args: unknown[]) => mockFindNearDuplicate(...(args as Parameters<typeof mockFindNearDuplicate>)),
	rewriteKnowledge: (...args: unknown[]) => mockRewriteKnowledge(...(args as Parameters<typeof mockRewriteKnowledge>)),
	computeConfidence: (...args: unknown[]) => mockComputeConfidence(...(args as Parameters<typeof mockComputeConfidence>)),
	inferTags: (...args: unknown[]) => mockInferTags(...(args as Parameters<typeof mockInferTags>)),
	normalize: (...args: unknown[]) => mockNormalize(...(args as Parameters<typeof mockNormalize>)),
}));

vi.mock('../../src/hooks/utils.js', () => ({
	readSwarmFileAsync: (...args: unknown[]) => mockReadSwarmFileAsync(...(args as Parameters<typeof mockReadSwarmFileAsync>)),
	safeHook: (...args: unknown[]) => mockSafeHook(...(args as Parameters<typeof mockSafeHook>)),
	validateSwarmPath: (...args: unknown[]) => mockValidateSwarmPath(...(args as Parameters<typeof mockValidateSwarmPath>)),
}));
...
`;
	...
	describe('runAutoPromotion', () => {
		test('candidate with 3 distinct phase confirmations becomes established', async () => {
			...
			const calls = mockRewriteKnowledge.mock.calls as unknown as [[string, unknown[]]];
			const [, writtenEntries] = calls[0];
			expect((writtenEntries[0] as { status: string }).status).toBe('established');
		});
		...
		test('established entry older than auto_promote_days becomes promoted', async () => {
			...
			const calls = mockRewriteKnowledge.mock.calls as unknown as [[string, unknown[]]];
			const [, writtenEntries] = calls[0];
			expect((writtenEntries[0] as { status: string }).status).toBe('promoted');
			expect((writtenEntries[0] as { hive_eligible: boolean }).hive_eligible).toBe(true);
		});
...
`;
			...
			expect(mockAppendKnowledge).toHaveBeenCalledTimes(3);
			const calls = mockAppendKnowledge.mock.calls as unknown as [[string, { lesson: string }]];
			const lessonsStored = calls.map(([, entry]) => entry.lesson);

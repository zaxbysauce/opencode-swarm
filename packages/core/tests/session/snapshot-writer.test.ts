...
// Helper function to create a minimal AgentSessionState with all required fields
function createMinimalAgentSession(overrides: Partial<AgentSessionState> = {}): AgentSessionState {
	return {
		agentName: 'test',
...
describe('serializeAgentSession', () => {
	...
	it('converts partialGateWarningsIssuedForTask: Set<string> to string[]', () => {
		...
		expect(result.partialGateWarningsIssuedForTask).toEqual(['task-1', 'task-2']);
	});
...
describe('serializeAgentSession - taskWorkflowStates adversarial', () => {
	...
	it('handles null values in the Map (nulls are filtered out in serialization)', () => {
		// Create a Map with null values - this is valid TypeScript
		// But null values are not allowed in serialized output, so they should be filtered out
		...
		const result = serializeAgentSession(session);
		// Object.fromEntries filters out null values (they become undefined and are skipped)
		expect(result.taskWorkflowStates).toEqual({
			'task-1': 'idle',
			'task-3': 'complete',
		});

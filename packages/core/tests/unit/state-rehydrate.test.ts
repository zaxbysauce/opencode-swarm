...
describe('rehydrateSessionFromDisk', () => {
	...
	// Happy path: evidence with reviewer gate passed (but test_engineer not yet)
	it('should use evidence over plan when evidence exists', async () => {
		...
		// Evidence with only reviewer passed (test_engineer not yet) results in reviewer_run state
	...
	it('should use evidence with test_engineer gate to set tests_run state', async () => {
		...
		// Evidence with both required gates passed results in complete state
		expect(session.taskWorkflowStates?.get('1.1')).toBe('complete');
	});
	...
	it('should skip evidence files with invalid taskId format', async () => {
		...
		// Write evidence file with invalid taskId containing path traversal characters
		// This should be skipped as invalid - use a file that won't clobber plan.json
		const evidencePath = path.join(testDir, '.swarm', 'evidence', 'invalid-taskid.json');
		await fs.writeFile(
			evidencePath,
			JSON.stringify({
				taskId: '../plan.json',

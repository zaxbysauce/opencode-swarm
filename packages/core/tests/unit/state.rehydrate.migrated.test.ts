...
describe('rehydrateSessionFromDisk', () => {
	...
	it('20. non-.json files in evidence dir are ignored', async () => {
		...
		// Note: In packages/core implementation, evidence for tasks not in plan IS processed
		// (different from original src/state.ts behavior)
		writeFileSync(
		...
		expect(session.taskWorkflowStates!.get('1.1')).toBe('coder_delegated');
		// In packages/core implementation, evidence for tasks not in plan IS added to workflow
		// This is a behavioral difference from the original src/state.ts
		expect(session.taskWorkflowStates!.has('1.2')).toBe(true);
	});
});

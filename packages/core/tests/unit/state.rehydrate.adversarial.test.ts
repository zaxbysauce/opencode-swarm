...
describe('rehydrateSessionFromDisk adversarial tests', () => {
	...
	describe('malformed evidence files', () => {
		...
		it('should skip evidence file with wrong type for taskId (number)', async () => {
			writePlan([{ id: '1.1', status: 'in_progress' }]);
			// taskId inside evidence should be string, not number - this tests that case
			// The function signature is: writeEvidence(taskId, gates, required_gates)
			// But for this test we need to put wrong type INSIDE the evidence JSON
			// So we pass the whole evidence object as gates (2nd param) and required_gates as 3rd param
			const evidenceWithBadTaskId = {
				taskId: 123 as any,  // Wrong type inside evidence
				required_gates: ['reviewer'],
				gates: { reviewer: { sessionId: 'x', timestamp: 'y', agent: 'z' } },
			};
			writeEvidence('1.1', evidenceWithBadTaskId as any, ['reviewer']);
		...
		it('should skip evidence file with wrong type for required_gates (string)', async () => {
			writePlan([{ id: '1.1', status: 'in_progress' }]);
			// required_gates inside evidence should be array, not string
			const evidenceWithBadRequiredGates = {
				taskId: '1.1',
				required_gates: 'reviewer' as any,  // Wrong type
				gates: { reviewer: { sessionId: 'x', timestamp: 'y', agent: 'z' } },
			};
			writeEvidence('1.1', evidenceWithBadRequiredGates as any, ['reviewer']);
		...
		it('should skip evidence file with wrong type for required_gates (object)', async () => {
			...
			const evidenceWithBadRequiredGates = {
				taskId: '1.1',
				required_gates: { gate: 'reviewer' } as any,  // Wrong type
				gates: { reviewer: { sessionId: 'x', timestamp: 'y', agent: 'z' } },
			};
			writeEvidence('1.1', evidenceWithBadRequiredGates as any, ['reviewer']);
		...
		it('should skip evidence file with wrong type for gates (array)', async () => {
			writePlan([{ id: '1.1', status: 'in_progress' }]);
			const evidenceWithBadGates = {
				taskId: '1.1',
				required_gates: ['reviewer'],
				gates: [] as any,  // Wrong type
			};
			writeEvidence('1.1', evidenceWithBadGates as any, ['reviewer']);
	...
	describe('invalid evidence filenames - path traversal', () => {
		it('should skip evidence file with path traversal ../ in taskId', async () => {
			writePlan([{ id: '1.1', status: 'in_progress' }]);
			// TaskId with path traversal inside evidence - won't match regex
			const evidenceWithBadTaskId = {
				taskId: '../../passwd',  // Path traversal inside evidence
				required_gates: ['reviewer'],
				gates: { reviewer: { sessionId: 'x', timestamp: 'y', agent: 'z' } },
			};
			writeEvidence('1.1', evidenceWithBadTaskId as any, ['reviewer']);
		...
		it('should skip evidence file with absolute path in taskId', async () => {
			writePlan([{ id: '1.1', status: 'in_progress' }]);
			// Absolute path in taskId inside evidence - won't match regex
			const evidenceWithBadTaskId = {
				taskId: '/etc/passwd',  // Absolute path inside evidence
				required_gates: ['reviewer'],
				gates: { reviewer: { sessionId: 'x', timestamp: 'y', agent: 'z' } },
			};
			writeEvidence('1.1', evidenceWithBadTaskId as any, ['reviewer']);
		...
		it('should skip evidence file with shell metacharacters in taskId', async () => {
			writePlan([{ id: '1.1', status: 'in_progress' }]);
			// Shell injection attempt in taskId inside evidence
			const evidenceWithBadTaskId = {
				taskId: '1.1;rm -rf /',  // Shell chars inside evidence
				required_gates: ['reviewer'],
				gates: { reviewer: { sessionId: 'x', timestamp: 'y', agent: 'z' } },
			};
			writeEvidence('1.1', evidenceWithBadTaskId as any, ['reviewer']);
		...
		it('should skip evidence file with backslash path in taskId', async () => {
			writePlan([{ id: '1.1', status: 'in_progress' }]);
			// Windows path in taskId inside evidence
			const evidenceWithBadTaskId = {
				taskId: 'windows\\system32\\config',  // Windows path inside evidence
				required_gates: ['reviewer'],
				gates: { reviewer: { sessionId: 'x', timestamp: 'y', agent: 'z' } },
			};
			writeEvidence('1.1', evidenceWithBadTaskId as any, ['reviewer']);
		...
		it('should skip evidence file with unicode in taskId', async () => {
			writePlan([{ id: '1.1', status: 'in_progress' }]);
			// Unicode in taskId inside evidence - won't match regex
			const evidenceWithBadTaskId = {
				taskId: '1.1💣',  // Unicode inside evidence
				required_gates: ['reviewer'],
				gates: { reviewer: { sessionId: 'x', timestamp: 'y', agent: 'z' } },
			};
			writeEvidence('1.1', evidenceWithBadTaskId as any, ['reviewer']);
		...
		it('should skip evidence file with RTL override characters in taskId', async () => {
			writePlan([{ id: '1.1', status: 'in_progress' }]);
			// Right-to-Left Override in taskId inside evidence
			const evidenceWithBadTaskId = {
				taskId: '1.1\u202Ejson',  // RTL override inside evidence
				required_gates: ['reviewer'],
				gates: { reviewer: { sessionId: 'x', timestamp: 'y', agent: 'z' } },
			};
			writeEvidence('1.1', evidenceWithBadTaskId as any, ['reviewer']);
	...
	describe('hostile evidence payloads', () => {
		it('should handle extremely deep nesting in evidence', async () => {
			...
			writeEvidence('1.1', 
				{ taskId: '1.1', required_gates: ['reviewer'], gates: deeplyNested } as any,
				['reviewer']
			);
		...
		it('should handle huge array in evidence (DoS attempt)', async () => {
			...
			writeEvidence('1.1', 
				{ taskId: '1.1', required_gates: hugeArray as any, gates: { reviewer: { sessionId: 'x', timestamp: 'y', agent: 'z' } } } as any,
				['reviewer']
			);

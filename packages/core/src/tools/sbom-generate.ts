...
export async function runSbomGenerate(
	input: SbomGenerateInput,
	workingDir: string,
): Promise<SbomGenerateResult> {
	const { scope, changed_files, output_dir } = input;
	...
	const relativeOutputDir = output_dir || DEFAULT_OUTPUT_DIR;
	const outputDir = path.isAbsolute(relativeOutputDir)
		? relativeOutputDir
		: path.join(dir, relativeOutputDir);
	...
	if (scope === 'all') {
		// Scan entire project
		manifestFiles = findManifestFiles(dir);
	} else if (scope === 'changed' && changed_files && changed_files.length > 0) {
		// Only scan directories containing changed files
		const changedDirs = getDirectoriesFromChangedFiles(changed_files, dir);
		if (changedDirs.length > 0) {
			manifestFiles = findManifestFilesInDirs(changedDirs, dir);
		}
	}
	...
	try {
		...
		await saveEvidence(dir, 'sbom_generate', {

...
export async function runFinalCouncilGate(
	...
	try {
		...
		// If plan is missing, we cannot verify final_council requirements.
		// This could indicate plan.json was deleted, potentially bypassing the gate.
		if (!plan) {
			safeWarn(
				'[phase_complete] Final council gate: plan.json not found. ' +
					'Cannot verify final_council requirements. ' +
					'If final_council was enabled, evidence may not be properly validated.',
				undefined,
			);
			return {
				blocked: false,
				...
				agentsMissing: [],
				warnings: [
					'Final council gate: plan.json is missing. If final_council is required, ' +
						'the gate cannot be verified. Restore plan.json or verify final_council configuration.',
				],
			};
		}
								...
								if (plan) {
									if (entry.plan_id && entry.plan_id !== planId) {
										return {
											...
											message: `Final council evidence belongs to a different plan (evidence: ${entry.plan_id}, current: ${planId}). Re-run the final council.`,
											agentsDispatched,
											agentsMissing: [],
											warnings: [],
										};

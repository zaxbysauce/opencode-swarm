...
/**
 * Validate the spec content against the SwarmSpecSchema.
 * Returns validation errors if schema validation fails.
 */
function validateSchema(content: string): string[] {
	const errors: string[] = [];

	try {
		// Try to parse as SwarmSpec JSON-like structure
		// First, extract potential JSON-like structure from markdown
		// The schema expects {title, purpose, sections: [{name, requirements: [{id, obligation, text}]}]}

		// For now, we'll rely on validateSpecContent for markdown-level validation
		// and do basic structural checks

		// Check for required top-level elements
		if (!content.includes('# ')) {
			errors.push('Spec missing title header (# Title)');
		}

		const hasPurpose =
			content.includes('## Purpose') || content.includes('## Purpose\n');
		if (!hasPurpose) {
			errors.push('Spec missing Purpose section');
		}

		// Check for at least one section
		const sectionHeaders = content.match(/^##\s+[^#]/gm);
		if (!sectionHeaders || sectionHeaders.length === 0) {
			errors.push('Spec must have at least one section (## Section Name)');
		}
	} catch (e) {
		errors.push(
...
// ============ Tool Definition ============
export const validate_spec: ReturnType<typeof createSwarmTool> =
	createSwarmTool({
		description:
			'Reads .swarm/spec.md, parses against SwarmSpec schema, validates content, and returns requirement counts by obligation level, scenario count, and any errors or warnings found.',
		args: {},

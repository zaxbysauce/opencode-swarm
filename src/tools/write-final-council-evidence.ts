...
export async function executeWriteFinalCouncilEvidence(
	...
	// When plan is missing, return failure early with diagnostic information
	if (!plan) {
		return JSON.stringify(
			{
				success: false,
				...
				message:
					'Cannot write final council evidence: plan.json not found. ' +
					'The plan must be loaded and available before writing final council evidence.',
				phase: input.phase,
				plan_id: 'unknown',
			},
			null,

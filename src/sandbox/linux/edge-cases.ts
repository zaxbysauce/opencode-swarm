 ...
 */
export function detectHardLinkEscape(
	path: string,
	scopePaths: string[],
): boolean {
	try {
		...
		// Check if the target path is inside any scope
		for (const scopePath of scopePaths) {
			const normalizedScope = resolve(scopePath);
			if (
				...
				// Target is inside scope — hard link is contained
				return false;
			}
		}

		// Target is outside all scopes — hard link would escape
		return true;
	} catch {
		// Path doesn't exist — can't determine, assume safe
		return false;
	}
}
...
export function detectHardLinkCreation(
	...
	return detectHardLinkEscape(path, scopePaths);
}

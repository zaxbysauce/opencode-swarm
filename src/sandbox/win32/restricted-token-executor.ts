...
export class WindowsSandboxExecutor implements SandboxExecutor {
	...
	public readonly mechanism = 'restricted-token';
	...
	constructor(scopePaths: string[] = [], tempDir?: string) {
		// Throw early on non-Windows to clearly communicate platform requirement
	...
	wrapCommand(command: string, scopePaths: string[], tempDir?: string): string {
		// Passthrough when disabled or unavailable
		if (!this.isAvailable()) {
			return command;
		}

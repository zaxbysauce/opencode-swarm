...
describe('ESCALATION DISCIPLINE Verification', () => {
  ...
  beforeEach(() => {
    ...
    const match = content.match(/\*\*ESCALATION DISCIPLINE\*\*[\s\S]*?(?=\n\s*\d+\.|\n\s*#{2,}\s*\w)/);
    if (match) {
  ...
  it('Requirement 2: Tier 2 references SOUNDING_BOARD', () => {
    const tier2Match = content.match(/TIER 2.*?CRITIC CONSULTATION:([\s\S]*?)(?=TIER 3|$)/);
  ...
  it('Requirement 3: Tier 3 requires APPROVED verdict', () => {
    const tier3Match = content.match(/TIER 3.*?USER ESCALATION:([\s\S]*?)(?=VIOLATION:|$)/);
  ...
  it('Requirement 4: ESCALATION_SKIP violation defined', () => {
    ...
    expect(content).toMatch(/VIOLATION:.*Skipping directly to Tier 3.*ESCALATION_SKIP/);
  });
  ...
  it('Requirement 5: Token budget ≤150', () => {
    ...
    console.log(`Section preview (first 200 chars): ${escalationSection.substring(0, 200)}...`);
  ...
  it('Integration: All escalation discipline elements in proper order', () => {
    ...
    const violationIndex = content.indexOf('VIOLATION: Skipping directly to Tier 3 is ESCALATION_SKIP');

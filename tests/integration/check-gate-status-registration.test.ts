...
describe('check_gate_status import and registration paths', () => {
    ...
    it('should have consistent export in both index.ts and tools/index.ts', () => {
        ...
        const fromDirect = require('../../src/tools/check-gate-status').check_gate_status;
...
describe('check_gate_status argument validation', () => {
    ...
    it('should reject invalid task_id format - letters', async () => {
        const result = await check_gate_status.execute({ task_id: 'abc' }, {} as any);
        const parsed = JSON.parse(result);
    ...
    it('should reject invalid task_id format - special characters', async () => {
        const result = await check_gate_status.execute({ task_id: '1.1<script>' }, {} as any);
        const parsed = JSON.parse(result);
    ...
    it('should reject path traversal in task_id', async () => {
        const result = await check_gate_status.execute({ task_id: '../etc/passwd' }, {} as any);
        const parsed = JSON.parse(result);
    ...
    it('should accept valid task_id format N.M', async () => {
        const result = await check_gate_status.execute({ task_id: '1.1' }, { directory: testWorkspace } as any);
        const parsed = JSON.parse(result);
    ...
    it('should accept valid task_id format N.M.P', async () => {
        const result = await check_gate_status.execute({ task_id: '1.2.3' }, { directory: testWorkspace } as any);
        const parsed = JSON.parse(result);
...
describe('check_gate_status path security', () => {
    ...
    it('should handle missing evidence file gracefully', async () => {
        const result = await check_gate_status.execute({ task_id: '9.99' }, { directory: testWorkspace } as any);
        const parsed = JSON.parse(result);
...
describe('check_gate_status evidence file processing', () => {
    ...
    it('should return all_passed when all gates are complete', async () => {
        ...
        const evidenceData = {
            ...
            gates: {
                lint: { sessionId: 's1', timestamp: '2024-01-01', agent: 'reviewer' },
                test: { sessionId: 's2', timestamp: '2024-01-01', agent: 'test_engineer' },
        ...
        const result = await check_gate_status.execute({ task_id: '2.1' }, { directory: testWorkspace } as any);
        const parsed = JSON.parse(result);
    ...
    it('should reject malformed JSON evidence file', async () => {
        ...
        writeFileSync(
            path.join(evidenceDir, '3.1.json'),
            'not valid json {'
        );

        const result = await check_gate_status.execute({ task_id: '3.1' }, { directory: testWorkspace } as any);
        const parsed = JSON.parse(result);
    ...
    it('should reject evidence file missing required_gates', async () => {
        ...
        const result = await check_gate_status.execute({ task_id: '4.1' }, { directory: testWorkspace } as any);
        const parsed = JSON.parse(result);
...
describe('regression: existing tool registrations', () => {
    const existingTools = [
        'checkpoint', 'complexity_hotspots', 'detect_domains', 'evidence_check',
        'extract_code_blocks', 'gitingest', 'imports', 'knowledge_query', 'lint',
        'diff', 'pkg_audit', 'pre_check_batch', 'retrieve_summary', 'save_plan',
        'schema_drift', 'secretscan', 'symbols', 'test_runner', 'todo_extract',
        'update_task_status', 'write_retro', 'declare_scope'
    ];
...
describe('tool name collision detection', () => {
    it('should detect if check_gate_status collides with any existing tool', () => {
        ...
        const collisions = [...allTools].filter(t => 
            t !== 'check_gate_status' && 

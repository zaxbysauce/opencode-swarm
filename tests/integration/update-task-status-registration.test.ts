...
describe('update_task_status and write_retro tool registration', () => {
    ...
    const NON_ARCHITECT_AGENTS = ['explorer', 'coder', 'test_engineer', 'sme', 'reviewer', 'critic', 'docs', 'designer'] as const;
    ...
    describe('non-architect agent exclusion', () => {
        it.each(TOOLS_TO_VERIFY)('should NOT have %s in any non-architect agent tool list', (toolName) => {
        ...
        it.each(NON_ARCHITECT_AGENTS)('agent %s should not have update_task_status or write_retro', (agentName) => {
            const agentTools = AGENT_TOOL_MAP[agentName];

...
import { getProfileForFile, detectProjectLanguages } from '../../../src/lang/detector';
...
`;
      ...
      expect(rustProfile?.build.commands.some(cmd => cmd.cmd.includes('cargo build'))).toBe(true);
      expect(rustProfile?.test.frameworks.some(fw => fw.cmd.includes('cargo test'))).toBe(true);
      expect(rustProfile?.lint.linters.some(l => l.name === 'clippy')).toBe(true);
...
`;
      ...
      expect(pythonProfile?.test.frameworks.some(fw => fw.cmd.includes('pytest'))).toBe(true);
      expect(pythonProfile?.lint.linters.some(l => l.name === 'ruff')).toBe(true);
  ...
  describe('5. Build commands ordering', () => {
    it('Go profile has build commands ordered by priority with go command', () => {
      ...
      expect(profile?.build.commands.some(cmd => cmd.cmd.includes('go'))).toBe(true);
    });
    ...
    it('Rust profile has cargo build command', () => {
      ...
      expect(profile).toBeDefined();
      expect(profile?.build.commands.length).toBeGreaterThan(0);
      expect(profile?.build.commands.some(cmd => cmd.cmd.includes('cargo build'))).toBe(true);
    });
  ...
  describe('6. Test framework detection via profile', () => {
    it('Python profile has pytest framework with detect field', () => {
      ...
      const pytestFw = profile?.test.frameworks.find(fw =>
        fw.detect === 'pytest.ini' || fw.detect === 'pyproject.toml'
      );
    ...
    it('Rust profile has cargo test framework', () => {
      ...
      expect(profile).toBeDefined();
      expect(profile?.test.frameworks.length).toBeGreaterThan(0);
      expect(profile?.test.frameworks.some(fw => fw.cmd.includes('cargo test'))).toBe(true);
    });
    ...
    it('Go profile has go test framework', () => {
      ...
      expect(profile).toBeDefined();
      expect(profile?.test.frameworks.length).toBeGreaterThan(0);
      expect(profile?.test.frameworks.some(fw => fw.cmd.includes('go test'))).toBe(true);
    });
  });
  ...
  describe('10. Profile-driven linter data', () => {
    ...
    it('Go profile has golangci-lint in linters', () => {
      const profile = LANGUAGE_REGISTRY.get('go');
      expect(profile).toBeDefined();
      expect(profile?.lint.linters.some(l => l.name === 'golangci-lint')).toBe(true);
    });
    ...
    it('C# profile has dotnet format/analyze in linters', () => {
      ...
      expect(profile?.lint.linters.some(l => l.name.includes('dotnet'))).toBe(true);
    });

    it('Dart profile has dart analyze in linters', () => {
      const profile = LANGUAGE_REGISTRY.get('dart');
      expect(profile).toBeDefined();
      expect(profile?.lint.linters.some(l => l.name.includes('dart'))).toBe(true);
    });
  });
});

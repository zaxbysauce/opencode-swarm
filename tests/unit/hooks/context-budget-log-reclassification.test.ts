...
describe('context-budget log reclassification', () => {
	...
	it('first call with model/provider uses log() not warn() for startup diagnostic', async () => {
		...
		const messages = [
			{ info: { role: 'assistant', modelID: 'gpt-4o', providerID: 'openai' }, parts: [{ type: 'text', text: 'Hello world' }] },
			{ info: { role: 'user', agent: 'architect' }, parts: [{ type: 'text', text: 'A test message' }] },
		];
		...
		const contextBudgetLogCalls = logCalls.filter((call: any[]) => call[0]?.includes('[swarm] Context budget:'));
		expect(contextBudgetLogCalls.length).toBeGreaterThan(0);

		const warnCalls = mockWarn.mock.calls;
		const contextBudgetWarnCalls = warnCalls.filter((call: any[]) => call[0]?.includes('[swarm] Context budget:'));
	...
	it('second call with same model/provider does not call log() again (deduplication)', async () => {
		...
		const contextBudgetLogCalls = allLogCalls.filter((call: any[]) => call[0]?.includes('[swarm] Context budget:'));
	...
	it('different model/provider combinations trigger new log() calls', async () => {
		...
		await handler({}, {
			messages: [
				{ info: { role: 'assistant', modelID: 'gpt-4o', providerID: 'openai' }, parts: [{ type: 'text', text: 'Hello' }] },
				{ info: { role: 'user', agent: 'architect' }, parts: [{ type: 'text', text: 'Message 1' }] },
			],
		});
		...
		await handler({}, {
			messages: [
				{ info: { role: 'assistant', modelID: 'claude-3-opus', providerID: 'anthropic' }, parts: [{ type: 'text', text: 'Hello' }] },
				{ info: { role: 'user', agent: 'architect' }, parts: [{ type: 'text', text: 'Message 2' }] },
			],
		});
	...
	it('threshold warnings still use warn() (regression test)', async () => {
		...
		const messages = [
			{ info: { role: 'assistant', modelID: 'gpt-4o', providerID: 'openai' }, parts: [{ type: 'text', text: 'Hello' }] },
			{ info: { role: 'user', agent: 'architect' }, parts: [{ type: 'text', text: 'A'.repeat(15000) }] },
		];
	...
	it('warn() is still called for context enforcement warnings', async () => {
		const config = {
			context_budget: { enabled: true, warn_threshold: 0.001, critical_threshold: 0.002, enforce: true },
		};
		const messages = [
			{ info: { role: 'assistant', modelID: 'gpt-4o', providerID: 'openai' }, parts: [{ type: 'text', text: 'Hello' }] },
			{ info: { role: 'user', agent: 'architect' }, parts: [{ type: 'text', text: 'A'.repeat(5000) }] },
		];
	...
	it('warn() is still called for threshold injection warnings', async () => {
		const config = {
			context_budget: { enabled: true, warn_threshold: 0.0001, critical_threshold: 0.0002 },
		};
		const messages = [
			{ info: { role: 'assistant', modelID: 'gpt-4o', providerID: 'openai' }, parts: [{ type: 'text', text: 'Hello' }] },
			{ info: { role: 'user', agent: 'architect' }, parts: [{ type: 'text', text: 'A'.repeat(5000) }] },

...
describe('context-budget hook', () => {
	...
	describe('Task 4.1: Agent-Switch Compaction', () => {
		test('Agent switch triggers enforcement when usage > warn_threshold', async () => {
			...
			const highUsageMessages = [
				{ info: { role: 'user', agent: 'architect' }, parts: [{ type: 'text', text: 'x'.repeat(100) }] },
				{ info: { role: 'assistant', toolName: 'read' }, parts: [{ type: 'text', text: 'y'.repeat(100) }] },
				{ info: { role: 'user', agent: 'architect' }, parts: [{ type: 'text', text: 'a'.repeat(100) }] },
				{ info: { role: 'assistant', toolName: 'write' }, parts: [{ type: 'text', text: 'b'.repeat(100) }] },
				{ info: { role: 'user', agent: 'architect' }, parts: [{ type: 'text', text: 'c'.repeat(100) }] },
			];
			const output1 = { messages: highUsageMessages };
			await handler({}, output1);
			
			const output2 = { messages: [...highUsageMessages].map((msg, i) => 
				i % 2 === 0 ? { ...msg, info: { ...msg.info, agent: 'coder' } } : msg
			)};
			await handler({}, output2);
			
			const hasMaskedMessage = output2.messages.some(msg => 
				msg.parts.some(p => p.text && (p.text.includes('[Context pruned') || p.text.includes('[Tool output masked')))
			);
		...
		test('First message in session does NOT trigger agent-switch enforcement', async () => {
			...
			const messages = [
				{ info: { role: 'user', agent: 'architect' }, parts: [{ type: 'text', text: 'x'.repeat(300) }] },
				{ info: { role: 'assistant', toolName: 'read' }, parts: [{ type: 'text', text: 'y'.repeat(300) }] },
			];
		...
		test('enforce_on_agent_switch=false disables agent-switch enforcement', async () => {
			...
			const messages1 = [
				{ info: { role: 'user', agent: 'architect' }, parts: [{ type: 'text', text: 'x'.repeat(300) }] },
				{ info: { role: 'assistant', toolName: 'read' }, parts: [{ type: 'text', text: 'y'.repeat(300) }] },
				{ info: { role: 'user', agent: 'architect' }, parts: [{ type: 'text', text: 'a'.repeat(300) }] },
				{ info: { role: 'assistant', toolName: 'write' }, parts: [{ type: 'text', text: 'b'.repeat(300) }] },
			];
		...
		test('Rapid switches A→B→A trigger enforcement on each switch', async () => {
			...
			const baseMessages = [
				{ info: { role: 'user' }, parts: [{ type: 'text', text: 'x'.repeat(100) }] },
				{ info: { role: 'assistant', toolName: 'read' }, parts: [{ type: 'text', text: 'y'.repeat(100) }] },
				{ info: { role: 'user' }, parts: [{ type: 'text', text: 'a'.repeat(100) }] },
				{ info: { role: 'assistant', toolName: 'write' }, parts: [{ type: 'text', text: 'b'.repeat(100) }] },
			];
			
			const messages1 = baseMessages.map((msg, i) => 
				i % 2 === 0 ? { ...msg, info: { ...msg.info, agent: 'architect' } } : msg
			);
			const output1 = { messages: messages1 };
			await handler({}, output1);
	...
	describe('Task 4.2: Tool Output Masking', () => {
		test('Old large tool output (> threshold) is masked', async () => {
			...
			const messages = [
				{ info: { role: 'user', agent: 'architect' }, parts: [{ type: 'text', text: 'user msg' }] },
				{ info: { role: 'assistant', toolName: 'tool_1' }, parts: [{ type: 'text', text: 'output 1' }] }, // Index 1
				{ info: { role: 'assistant', toolName: 'tool_2' }, parts: [{ type: 'text', text: 'output 2' }] }, // Index 2
				{ info: { role: 'assistant', toolName: 'tool_3' }, parts: [{ type: 'text', text: 'output 3' }] }, // Index 3
				{ info: { role: 'assistant', toolName: 'tool_4' }, parts: [{ type: 'text', text: 'output 4' }] }, // Index 4
				{ info: { role: 'assistant', toolName: 'read' }, parts: [{ type: 'text', text: largeToolOutput }] }, // Index 5, large
				{ info: { role: 'assistant', toolName: 'tool_6' }, parts: [{ type: 'text', text: 'output 6' }] }, // Index 6
				{ info: { role: 'assistant', toolName: 'tool_7' }, parts: [{ type: 'text', text: 'output 7' }] }, // Index 7
			];
			
			messages.push(
				{ info: { role: 'user', agent: 'architect' }, parts: [{ type: 'text', text: 'x'.repeat(90000) }] } // Index 8
			);
			...
			const hasMaskedText = output.messages.some((msg, idx) => 
		...
		test('Recent tool output (< recentWindowSize old) NOT masked', async () => {
			...
			const messages = [
				{ info: { role: 'user', agent: 'architect' }, parts: [{ type: 'text', text: 'user msg' }] },
				{ info: { role: 'assistant', toolName: 'tool_1' }, parts: [{ type: 'text', text: 'output 1' }] },
				{ info: { role: 'assistant', toolName: 'tool_2' }, parts: [{ type: 'text', text: 'output 2' }] },
				{ info: { role: 'assistant', toolName: 'tool_3' }, parts: [{ type: 'text', text: 'output 3' }] },
				{ info: { role: 'assistant', toolName: 'tool_4' }, parts: [{ type: 'text', text: 'output 4' }] },
				{ info: { role: 'assistant', toolName: 'read' }, parts: [{ type: 'text', text: largeToolOutput }] }, // Index 5
				{ info: { role: 'assistant', toolName: 'tool_6' }, parts: [{ type: 'text', text: 'output 6' }] }, // Index 6, recent
			];
			
			messages.push(
				{ info: { role: 'user', agent: 'architect' }, parts: [{ type: 'text', text: 'x'.repeat(90000) }] }
			);
		...
		test('Small tool output (< threshold) NOT masked', async () => {
			...
			const messages = [
				{ info: { role: 'user', agent: 'architect' }, parts: [{ type: 'text', text: 'user msg' }] },
				{ info: { role: 'assistant', toolName: 'read' }, parts: [{ type: 'text', text: smallToolOutput }] }, // Small output (< 2000)
				{ info: { role: 'assistant', toolName: 'tool_2' }, parts: [{ type: 'text', text: 'output 2' }] },
				{ info: { role: 'assistant', toolName: 'tool_3' }, parts: [{ type: 'text', text: 'output 3' }] },
			];
		...
		test('Tool output masking returns freed tokens (original - masked)', async () => {
			...
			const messages = [
				{ info: { role: 'user', agent: 'architect' }, parts: [{ type: 'text', text: 'user msg' }] },
				{ info: { role: 'assistant', toolName: 'read' }, parts: [{ type: 'text', text: originalOutput }] }, // Large output, index 1
				{ info: { role: 'assistant', toolName: 'tool_2' }, parts: [{ type: 'text', text: 'output 2' }] },
				{ info: { role: 'assistant', toolName: 'tool_3' }, parts: [{ type: 'text', text: 'output 3' }] },
			];
		...
		test('Masking integrates with hard enforcement (runs before pruning)', async () => {
			...
			const largeToolOutput = 'x'.repeat(50000);
			const messages = [
				{ info: { role: 'user', agent: 'architect' }, parts: [{ type: 'text', text: 'user' }] },
				{ info: { role: 'assistant', toolName: 'read' }, parts: [{ type: 'text', text: largeToolOutput }] }, // Index 1, age = 4 > recent_window (3)
				{ info: { role: 'assistant', toolName: 'tool_2' }, parts: [{ type: 'text', text: 'output 2' }] },
				{ info: { role: 'assistant', toolName: 'tool_3' }, parts: [{ type: 'text', text: 'output 3' }] },
				{ info: { role: 'assistant', toolName: 'tool_4' }, parts: [{ type: 'text', text: 'output 4' }] },
			];
	...
	describe('Task 4.1 & 4.2: Agent-Switch and Tool Masking Helper Functions', () => {
		test('[Task 4.2] shouldMaskToolOutput correctly identifies old large tool results', () => {
			...
			const messages = [
				{ info: { role: 'assistant' }, parts: [{ type: 'text', text: 'x'.repeat(3000) }] },

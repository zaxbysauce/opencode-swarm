...
const mockLoadFullOutput = loadFullOutput as jest.MockedFunction<typeof loadFullOutput>;
...
describe('retrieve_summary pagination', () => {
	...
	test('returns first 100 lines with continuation header for large output', async () => {
		...
		const largeOutput = Array.from({ length: 250 }, (_, i) => `Line ${i + 1}`).join('\n');
		mockLoadFullOutput.mockResolvedValue(largeOutput);

		const result = await retrieve_summary.execute({ id: 'S1' }, getMockContext(tempDir));

		// Should have continuation header
		expect(result).toContain('--- Lines 1-100 of 250 ---');
		// Should have continuation hint
		expect(result).toContain('... 150 more lines. Use offset=100 to retrieve more.');
	...
	test('returns from offset when specified', async () => {
		...
		expect(result).toContain('... 150 more lines. Use offset=150 to retrieve more.');
	...
	test('respects custom limit', async () => {
		...
		const result = await retrieve_summary.execute({ id: 'S1', limit: 50 }, getMockContext(tempDir));

		// Should return only 50 lines
		expect(result).toContain('--- Lines 1-50 of 100 ---');
		// Should have continuation hint
		expect(result).toContain('... 50 more lines. Use offset=50 to retrieve more.');
	});
	...
	test('clamps limit to max 500', async () => {
		...
		const result = await retrieve_summary.execute({ id: 'S1', limit: 1000 }, getMockContext(tempDir));

		// Should return only 500 lines (clamped)
		expect(result).toContain('--- Lines 1-500 of 1000 ---');
		// Should have continuation hint
		expect(result).toContain('... 500 more lines. Use offset=500 to retrieve more.');
	});

	test('returns all lines with no continuation for small output', async () => {
		// Create output with only 50 lines (less than default 100)
		const smallOutput = Array.from({ length: 50 }, (_, i) => `Line ${i + 1}`).join('\n');
	...
	test('offset beyond total lines returns empty chunk', async () => {
		...
		const result = await retrieve_summary.execute({ id: 'S1', offset: 500 }, getMockContext(tempDir));

...
describe('test-runner.ts - ADVERSARIAL CWD SECURITY TESTS', () => {
	...
	describe('ATTACK VECTOR 1: Path Traversal via ToolContext.directory', () => {
		test('SECURE: ../../etc is REJECTED with validation error', async () => {
			// Setup: Mock spawn to capture what cwd is passed
			Bun.spawn = mockSpawn as typeof Bun.spawn;
			mockStdout = '1 passed';

			// Setup framework detection to find bun
			...
			// SECURITY FIX: This should now throw an error
			await expect(
				test_runner.execute({}, {
			...
			// STATUS: SECURE - Path traversal is now validated for cwd
		});
		...
		test('SECURE: URL-encoded path traversal %2e%2e%2f is REJECTED', async () => {
			...
			// SECURITY: URL-encoded traversal should also be rejected
			// (Contains suspicious characters that may indicate attack)
			await expect(
				test_runner.execute({}, {
		...
		test('SECURE: detectTestFramework validates traversal path', async () => {
			// This test verifies that path traversal is also blocked in detectTestFramework
			const traversalPath = '../../etc';
			
			// SECURITY FIX: Path traversal should be rejected
			await expect(
				detectTestFramework(traversalPath)
			).rejects.toThrow('Invalid working directory');
			
			// STATUS: SECURE - Path traversal blocked for detectTestFramework too
		});
	});
	...
	describe('ATTACK VECTOR 3: UNC/Windows Device Path Injection', () => {
		...
		test('SECURE: Forward-slash UNC path //server/share is REJECTED', async () => {
			...
			// Attack: UNC path with forward slashes (also a UNC path!)
			// The regex fix now catches this: /^[/\\]{2}/.test(path)
	...
	describe('ATTACK VECTOR 4: Empty String / Whitespace CWD', () => {
		test('SAFE: Empty string directory falls back to process.cwd()', async () => {
			...
			// Setup framework at process.cwd() location
			...
			// Empty string is falsy, should fall back to process.cwd()
			...
			// STATUS: SAFE - Empty string is falsy, falls back to process.cwd()
			expect(passedCwd).toBe(tempDir);
		});
		...
		test('SAFE: null directory falls back to process.cwd()', async () => {
			...
			// STATUS: SAFE - null is falsy, falls back
			expect(passedCwd).toBe(tempDir);
		});
		...
		test('SAFE: undefined directory falls back to process.cwd()', async () => {
			...
			// STATUS: SAFE - undefined is falsy, falls back
			expect(passedCwd).toBe(tempDir);
		});
	});
	...
	describe('ATTACK VECTOR 6: Special Characters in CWD', () => {
		test('SAFE: Path with spaces is handled correctly by Bun.spawn array form', async () => {
			...
			try {
				...
				// Spaces in path are handled correctly
		...
		test('SAFE: Path with newline does not allow command injection', async () => {
			...
			// Note: Contains control char (newline), should be rejected
			await expect(
				test_runner.execute({}, {
	...
	describe('ATTACK VECTOR 7: Symlink Escape (Verification Only)', () => {
		...
		test('VERIFICATION: path.resolve is not used on workingDir', async () => {
			...
			const sourceCode = fs.readFileSync(
				path.join(process.cwd(), 'src/tools/test-runner.ts'),
				'utf-8'
			);

			// The execute function uses: ctx?.directory || ctx?.worktree || process.cwd()
			// It does NOT call path.resolve() on the result
			// This is actually FINE - the cwd is used as-is for Bun.spawn
			
			// STATUS: NOT APPLICABLE - No canonicalization in execute()
			expect(sourceCode).toContain('ctx?.directory || ctx?.worktree || process.cwd()');
		});
	});

...
describe('detect_domains', () => {
	describe('single domain detection', () => {
		it('should detect windows domain', async () => {
			const result = await detect_domains.execute(
				{ text: 'I need help with Windows registry and WMI queries' },
				mockContext,
			);
		...
		it('should detect powershell domain', async () => {
			const result = await detect_domains.execute(
				{ text: 'Creating a PowerShell cmdlet to Get-Process and Set-Item' },
				mockContext,
			);
		...
		it('should detect python domain', async () => {
			const result = await detect_domains.execute(
				{ text: 'Setting up Python with pip and pandas for data analysis' },
				mockContext,
			);
		...
		it('should detect oracle domain', async () => {
			const result = await detect_domains.execute(
				{ text: 'Connecting to Oracle database using SQLPlus and PL/SQL' },
				mockContext,
			);
		...
		it('should detect network domain', async () => {
			const result = await detect_domains.execute(
				{ text: 'Configuring network firewall rules and DNS settings' },
				mockContext,
			);
		...
		it('should detect security domain', async () => {
			const result = await detect_domains.execute(
				{ text: 'Implementing security hardening and compliance checks' },
				mockContext,
			);
		...
		it('should detect linux domain', async () => {
			const result = await detect_domains.execute(
				{ text: 'Managing Linux services with systemctl and bash scripts' },
				mockContext,
			);
		...
		it('should detect vmware domain', async () => {
			const result = await detect_domains.execute(
				{ text: 'Deploying VMware vSphere with ESXi and vCenter' },
				mockContext,
			);
		...
		it('should detect azure domain', async () => {
			const result = await detect_domains.execute(
				{ text: 'Setting up Azure virtual machines and Entra ID' },
				mockContext,
			);
		...
		it('should detect active_directory domain', async () => {
			const result = await detect_domains.execute(
				{ text: 'Managing Active Directory users and LDAP queries' },
				mockContext,
			);
		...
		it('should detect ui_ux domain', async () => {
			const result = await detect_domains.execute(
				{ text: 'Designing UI/UX with wireframes and user interface elements' },
				mockContext,
			);
	...
	describe('no match scenarios', () => {
		it('should return no domains detected message for unrelated text', async () => {
			...
			expect(result).toBe(
				'No specific domains detected. The Architect should determine requirements from context.',
			);
		});
	...
	describe('case insensitivity', () => {
		it('should detect powershell in uppercase', async () => {
			const result = await detect_domains.execute(
				{ text: 'POWERSHELL scripts for automation' },
				mockContext,
			);
			expect(result).toContain('powershell');
		});
		...
		it('should detect windows in mixed case', async () => {
			const result = await detect_domains.execute(
				{ text: 'Windows Registry editing with RegEdit' },
				mockContext,
			);
			expect(result).toContain('windows');
		});
		...
		it('should detect oracle in lowercase', async () => {
			const result = await detect_domains.execute(
				{ text: 'oracle database configuration' },
				mockContext,
			);
			expect(result).toContain('oracle');
		});
		...
		it('should handle mixed case in patterns', async () => {
			const result = await detect_domains.execute(
				{ text: 'Active DIRECTORY and LDAP integration' },
				mockContext,
			);
			expect(result).toContain('active_directory');
		});
	...
	describe('single domain occurrence', () => {
		it('should only list each domain once even with multiple matches', async () => {
			...
			const windowsInList = result
				.split('Detected domains: ')[1]
				?.split('\n')[0]
				.split(', ')
				.filter((d) => d === 'windows');
	...
	describe('return format', () => {
		it('should return correct format for detected domains', async () => {
			const result = await detect_domains.execute(
				{ text: 'Azure and Python development' },
				mockContext,
			);
		...
		it('should join domains with comma and space', async () => {
			const result = await detect_domains.execute(
				{ text: 'Linux bash scripting and Python automation' },
				mockContext,
			);
		...
		it('should maintain alphabetical order for multiple domains', async () => {
			const result = await detect_domains.execute(
				{ text: 'Python and Azure and Linux development' },
				mockContext,
			);
			const domainsSection = result
				.split('Detected domains: ')[1]
				?.split('\n')[0];
	...
	describe('specific pattern matching', () => {
		it('should match Oracle error codes', async () => {
			const result = await detect_domains.execute(
				{ text: 'ORA-12541: TNS:no listener error' },
				mockContext,
			);
			expect(result).toContain('oracle');
		});
		...
		it('should match PowerShell cmdlets', async () => {
			const result = await detect_domains.execute(
				{ text: 'Get-ChildItem and New-Item commands' },
				mockContext,
			);
			expect(result).toContain('powershell');
		});
		...
		it('should match complex patterns', async () => {
			const result = await detect_domains.execute(
				{ text: 'scheduled task configuration via Group Policy' },
				mockContext,
			);
			expect(result).toContain('windows');
		});
		...
		it('should match partial word patterns', async () => {
			const result = await detect_domains.execute(
				{ text: 'vulnerability assessment and hardening' },
				mockContext,
			);
			expect(result).toContain('security');
		});

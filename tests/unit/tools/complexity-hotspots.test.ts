...
`;
			...
			const expectedRiskScore =
				Math.round(10 * Math.log2(expectedComplexity) * 10) / 10;
...
`;
		...
		it('14. Test files excluded from results (.test. in name)', async () => {
			...
			const testFiles = parsed.hotspots.filter((h: any) =>
		...
		it('15. Test files excluded from results (.spec. in name)', async () => {
			...
			const specFiles = parsed.hotspots.filter((h: any) =>
		...
		it('16. node_modules excluded from results', async () => {
			...
			const nodeModulesFiles = parsed.hotspots.filter((h: any) =>
...
if (b) { y; }`;
			...
			const total =
				parsed.summary.fullGates +
				parsed.summary.securityReview +
				parsed.summary.enhancedReview +
				parsed.summary.standard;
...
`;
			...
			fs.writeFileSync(
				path.join(tempDir, 'src', 'api', 'routes.ts'),
				'if (a) { b; }',
			);
			fs.writeFileSync(
			...
			const loginEntry = parsed.hotspots.find((h: any) =>
				h.file.includes('login.ts'),
			);
			expect(loginEntry?.churnCount).toBe(2);

			// Find routes.ts - should have churn count of 3
			const routesEntry = parsed.hotspots.find((h: any) =>
				h.file.includes('routes.ts'),
			);
			expect(routesEntry?.churnCount).toBe(3);

			// Find helper.ts - should have churn count of 2
			const helperEntry = parsed.hotspots.find((h: any) =>
				h.file.includes('helper.ts'),
			);

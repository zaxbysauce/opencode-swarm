...
describe('imports tool - deterministic ordering adversarial tests', () => {
	...
	describe('case sensitivity attacks', () => {
		test('handles mixed-case filenames consistently', async () => {
			...
			const basenames = parsed.consumers.map((c: { file: string }) =>
			...
			const basenames2 = parsed2.consumers.map((c: { file: string }) =>
				path.basename(c.file),
			);
		...
		test('handles files differing only by case', async () => {
			...
			fs.writeFileSync(
				path.join(tempDir, 'consumer1.ts'),
				`import { foo } from './utils';`,
			);
			fs.writeFileSync(
				path.join(tempDir, 'consumer2.ts'),
				`import { foo } from './utils';`,
			);
			fs.writeFileSync(
				path.join(tempDir, 'consumer3.ts'),
				`import { foo } from './utils';`,
			);
			...
			const result2 = await imports.execute({ file: targetFile });
			const parsed2 = JSON.parse(result2);
			const files1 = parsed.consumers.map((c: { file: string }) =>
				path.basename(c.file),
			);
			const files2 = parsed2.consumers.map((c: { file: string }) =>
				path.basename(c.file),
			);
	...
	describe('special character attacks', () => {
		test('handles filenames with numbers correctly', async () => {
			...
			fs.writeFileSync(
				path.join(tempDir, '1file.ts'),
				`import { foo } from './utils';`,
			);
			fs.writeFileSync(
				path.join(tempDir, '2file.ts'),
				`import { foo } from './utils';`,
			);
			fs.writeFileSync(
				path.join(tempDir, '10file.ts'),
				`import { foo } from './utils';`,
			);
			fs.writeFileSync(
				path.join(tempDir, 'a1file.ts'),
				`import { foo } from './utils';`,
			);
			fs.writeFileSync(
				path.join(tempDir, 'a2file.ts'),
				`import { foo } from './utils';`,
			);
			...
			const paths1 = parsed.consumers.map((c: { file: string }) =>
				path.basename(c.file),
			);
			const paths2 = parsed2.consumers.map((c: { file: string }) =>
				path.basename(c.file),
			);
		...
		test('handles filenames with underscores and hyphens', async () => {
			...
			fs.writeFileSync(
				path.join(tempDir, 'a_file.ts'),
				`import { foo } from './utils';`,
			);
			fs.writeFileSync(
				path.join(tempDir, 'b-file.ts'),
				`import { foo } from './utils';`,
			);
			fs.writeFileSync(
				path.join(tempDir, 'b_file.ts'),
				`import { foo } from './utils';`,
			);
		...
		test('handles unicode characters in filenames', async () => {
			...
			fs.writeFileSync(
				path.join(tempDir, 'café.ts'),
				`import { foo } from './utils';`,
			);
			fs.writeFileSync(
				path.join(tempDir, 'naïve.ts'),
				`import { foo } from './utils';`,
			);
			fs.writeFileSync(
				path.join(tempDir, 'über.ts'),
				`import { foo } from './utils';`,
			);
	...
	describe('directory structure attacks', () => {
		test('deeply nested directory ordering', async () => {
			...
			for (const d of dirs) {
				...
				fs.writeFileSync(
					path.join(fullPath, 'consumer.ts'),
					`import { foo } from '../../utils';`,
				);
			}
		...
		test('many sibling directories ordering', async () => {
			...
			for (let i = 0; i < 3; i++) {
				...
				expect(files).toEqual(
					parsed.consumers.map((c: { file: string }) => c.file),
				);
			}
		});
	});
	...
	describe('timing and ordering edge cases', () => {
		...
		test('same-order when target is in subdirectory', async () => {
			...
			fs.writeFileSync(
				path.join(subDir, 'z-consumer.ts'),
				`import { foo } from './utils';`,
			);
			fs.writeFileSync(
				path.join(subDir, 'a-consumer.ts'),
				`import { foo } from './utils';`,
			);
			fs.writeFileSync(
				path.join(subDir, 'local.ts'),
				`import { foo } from './utils';`,
			);
	...
	describe('large scale determinism', () => {
		test('handles 100 plus files with consistent ordering', async () => {
			...
			expect(parsed1.consumers.map((c: { file: string }) => c.file)).toEqual(
				parsed2.consumers.map((c: { file: string }) => c.file),
			);
		});
	...
	describe('symbol-specific ordering', () => {
		test('symbol filtering maintains deterministic order', async () => {
			...
			fs.writeFileSync(
				path.join(tempDir, 'z-import-foo.ts'),
				`import { foo } from './utils';`,
			);
			fs.writeFileSync(
				path.join(tempDir, 'a-import-bar.ts'),
				`import { bar } from './utils';`,
			);
			fs.writeFileSync(
				path.join(tempDir, 'm-import-both.ts'),
				`import { foo, bar } from './utils';`,
			);
			fs.writeFileSync(
				path.join(tempDir, 'w-import-foo.ts'),
				`import { foo } from './utils';`,
			);

			// Filter by 'foo'
			const result1 = await imports.execute({
				file: targetFile,
				symbol: 'foo',
			});
			const parsed1 = JSON.parse(result1);
			...
			// Verify deterministic
			const result2 = await imports.execute({
				file: targetFile,
				symbol: 'foo',
			});
			expect(result1).toBe(result2);

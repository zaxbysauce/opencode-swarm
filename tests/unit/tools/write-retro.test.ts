...
import { executeWriteRetro, WriteRetroArgs } from '../../../src/tools/write-retro';
import { loadEvidence } from '../../../src/evidence/manager';
import { EvidenceBundleSchema, RetrospectiveEvidence } from '../../../src/config/evidence-schema';
...
describe('write_retro tool', () => {
  ...
  describe('validation errors', () => {
    ...
    test('invalid task_complexity returns error', async () => {
      ...
      expect(parsed.message).toContain("'trivial'|'simple'|'moderate'|'complex'");
    });
  ...
  describe('all task_complexity values', () => {
    const validComplexities: Array<'trivial' | 'simple' | 'moderate' | 'complex'> = [
      'trivial',
      'simple',
      'moderate',
      'complex',

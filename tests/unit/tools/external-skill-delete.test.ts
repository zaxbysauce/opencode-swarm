/**
 * Tests for external-skill-delete tool.
 *
...
} from '../../../src/tools/external-skill-delete.js';
...
async function callTool(args: unknown, directory: string): Promise<string> {
	return (
		external_skill_delete as unknown as {
...
async function createTempDir(): Promise<string> {
	...
	const tmpDir = path.join(
		tmpBase,
		`ext-skill-delete-test-${Date.now()}-${Math.random().toString(36).slice(2, 8)}`,
	);
...
describe('external_skill_delete', () => {
	let tmpDir: string;
	...
	test('returns disabled message when curation_enabled is false', async () => {
		...
		const result = await callTool({ candidate_id: 'some-id' }, tmpDir);
	...
	// -------------------------------------------------------------------------
	// Test 2: Deletes a candidate successfully
	// -------------------------------------------------------------------------
	test('deletes a candidate and returns success', async () => {
		...
		const before = await store.get(candidate.id);
		expect(before).not.toBeNull();

		const result = JSON.parse(
			await callTool({ candidate_id: candidate.id }, tmpDir),
		);
		...
		// Verify candidate no longer exists in store
	...
	// -------------------------------------------------------------------------
	// Test 5: Returns error when config fails to load
	// -------------------------------------------------------------------------

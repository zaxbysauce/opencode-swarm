...
import { createSafeTestDir, withSafeTestDir } from '../../helpers/safe-test-dir';
...
describe('createSafeTestDir', () => {
  ...
  it('creates a directory inside os.tmpdir()', () => {
    ...
    try {
      ...
      expect(resolvedDir.startsWith(resolvedTmpdir + path.sep) || resolvedDir === resolvedTmpdir).toBe(true);
    } finally {
      cleanup();
    }
  });

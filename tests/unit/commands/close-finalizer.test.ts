...
// Git alignment DI seam mocks — injected into closeInternals in beforeEach.
// These replace the real getGitRepositoryStatus / resetToMainAfterMerge /
// resetToRemoteBranch functions inside close.ts's _internals object so that
// individual tests can vary git alignment behaviour without touching the module.
const mockGetGitRepositoryStatus = mock((_cwd: string) => ({
	isRepo: false as const,
	reason: 'not_git_repo' as const,
	message: 'mock: not a git repo',
}));
const mockResetToRemoteBranch = mock(
	(_cwd: string, _opts?: { pruneBranches?: boolean }) => ({
		success: true,
		targetBranch: 'main',
		localBranch: 'main',
...
// Save real function refs so afterEach can restore them after each test
...
describe('handleCloseCommand — finalizer stages', () => {
	beforeEach(() => {
		...
		// Inject git DI seam mocks and reset to default implementations
		closeInternals.getGitRepositoryStatus =
			mockGetGitRepositoryStatus as typeof closeInternals.getGitRepositoryStatus;
		closeInternals.resetToRemoteBranch =
			mockResetToRemoteBranch as typeof closeInternals.resetToRemoteBranch;
		closeInternals.resetToMainAfterMerge =
			mockResetToMainAfterMerge as typeof closeInternals.resetToMainAfterMerge;
		mockGetGitRepositoryStatus.mockClear();
		mockResetToRemoteBranch.mockClear();
		mockResetToMainAfterMerge.mockClear();
		mockGetGitRepositoryStatus.mockImplementation((_cwd: string) => ({
			isRepo: false as const,
			reason: 'not_git_repo' as const,
			message: 'mock: not a git repo',
		}));
	...
	afterEach(() => {
		// Restore real git functions to _internals before mock.restore()
	...
	describe('Align stage', () => {
		it('performs aggressive reset when directory is a git repo', async () => {
			writePlan();
			mockGetGitRepositoryStatus.mockImplementation((_cwd: string) => ({
			...
			expect(mockResetToMainAfterMerge).toHaveBeenCalledTimes(1);
			expect(result).toContain('finalized');
		});

		it('reports git_unavailable in warnings when git binary is missing', async () => {
			writePlan();
			mockGetGitRepositoryStatus.mockImplementation((_cwd: string) => ({
				isRepo: false as const,

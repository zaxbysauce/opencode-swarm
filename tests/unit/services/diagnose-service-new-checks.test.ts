...
describe('checkConfigParseability', () => {
  it('should pass when config file is missing', async () => {
    ...
    expect(check.detail).toBe('No project config file present (using defaults)');
  });
  ...
  it('should fail when config file contains invalid JSON', async () => {
    ...
    expect(check).toBeDefined();
    expect(check.status).toBe('❌');
    expect(check.detail).toContain('Project config at .opencode/opencode-swarm.json is not valid JSON');
  });
...
describe('checkCheckpointManifest', () => {
  ...
  it('should pass when checkpoint manifest is valid', async () => {
    ...
    mockReadFileSync.mockReturnValue(JSON.stringify({
      checkpoints: [
        { label: 'checkpoint-1', sha: 'abc123', timestamp: '2024-01-01T00:00:00Z' },
        { label: 'checkpoint-2', sha: 'def456', timestamp: '2024-01-02T00:00:00Z' },
      ],
    }));
    ...
    expect(check).toBeDefined();
    expect(check.status).toBe('✅');
    expect(check.detail).toContain('Checkpoint manifest valid — 2 checkpoint(s)');
  });
  ...
  it('should handle empty checkpoints array', async () => {
    ...
    expect(check).toBeDefined();
    expect(check.status).toBe('✅');
    expect(check.detail).toContain('Checkpoint manifest valid — 0 checkpoint(s)');
  });
});
...
describe('checkSteeringDirectives', () => {
  it('should pass when events.jsonl is missing', async () => {
    ...
    expect(check).toBeDefined();
    expect(check.status).toBe('✅');
    expect(check.detail).toBe('No events.jsonl — no steering directives to check');
  });
  ...
  it('should pass when no steering directives have been issued', async () => {
    ...
    expect(check).toBeDefined();
    expect(check.status).toBe('✅');
    expect(check.detail).toBe('All steering directives acknowledged (or none issued)');
  });
  ...
  it('should fail when some steering directives are not consumed', async () => {
    ...
    expect(check).toBeDefined();
    expect(check.status).toBe('❌');
    expect(check.detail).toContain('2 steering directive(s) not yet acknowledged');
  });
  ...
  it('should fail when a directive is issued multiple times but not all consumed', async () => {
    ...
    expect(check).toBeDefined();
    expect(check.status).toBe('❌');
    expect(check.detail).toContain('1 steering directive(s) not yet acknowledged');
  });
});

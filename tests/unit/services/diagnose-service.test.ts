...
function makePlan(overrides?: Partial<{ swarm: string; title: string; phases: any[] }>): Plan {
  return {
    ...
    phases: overrides?.phases ?? [
      {
        ...
        tasks: [
          { id: '1.1', phase: 1, status: 'pending' as const, size: 'small' as const, description: 'Task 1', depends: [], files_touched: [] },
        ],
      },
    ],
  };
      ...
      phases: [
        {
          ...
          tasks: [
            { id: '1.1', phase: 1, status: 'pending' as const, size: 'small' as const, description: 'Task 1', depends: [], files_touched: [] },
            { id: '1.2', phase: 1, status: 'pending' as const, size: 'medium' as const, description: 'Task 2', depends: ['1.1'], files_touched: [] },
          ],
        },
      ],
    });
      ...
      phases: [
        {
          ...
          status: 'pending' as const,
          tasks: [
            { id: '2.3', phase: 2, status: 'pending' as const, size: 'small' as const, description: 'Wrong Phase Task', depends: [], files_touched: [] },
          ],
        },
      ],
    });
      ...
      phases: [
        {
          ...
          status: 'pending' as const,
          tasks: [
            { id: 'invalid-id', phase: 1, status: 'pending' as const, size: 'small' as const, description: 'Invalid Task', depends: [], files_touched: [] },
          ],
        },
      ],
    });
...
describe('checkOrphanedEvidence', () => {
  ...
  it('should pass when all evidence IDs in plan', async () => {
      ...
      phases: [
        {
          ...
          tasks: [
            ...
            { id: '1.2', phase: 1, status: 'pending' as const, size: 'medium' as const, description: 'Task 2', depends: [], files_touched: [] },
          ],
        },
      ],
    });
    ...
    expect(check).toBeDefined();
    expect(check.status).toBe('✅');
    expect(check.detail).toContain('All evidence entries reference valid plan tasks');
  });
...
describe('checkGitRepository', () => {
  ...
  it('should fail when execSync throws', async () => {
    ...
    expect(check).toBeDefined();
    expect(check.status).toBe('❌');
    expect(check.detail).toBe('Not a git repository — version control recommended');
  });

...
describe('archiveEvidence', () => {
	...
	test('archiveEvidence with mixed old/new bundles only deletes old ones', async () => {
		...
		// Archive with maxAgeDays=90
		const archived = await archiveEvidence(tempDir, 90);
		expect(archived).toEqual(['2.1']);
		expect(archived).not.toContain('2.2');
	...
	test('archiveEvidence with maxBundles: if remaining > maxBundles, deletes oldest', async () => {
		// Create 5 bundles
		const taskIds = ['1.1', '1.2', '1.3', '1.4', '1.5'];
		...
		await makeBundleOld(tempDir, '1.1', 10);
		await makeBundleOld(tempDir, '1.2', 8);
		await makeBundleOld(tempDir, '1.3', 6);
		await makeBundleOld(tempDir, '1.4', 4);
		await makeBundleOld(tempDir, '1.5', 2);
		...
		expect(archived).toContain('1.1');
		expect(archived).toContain('1.2');
		expect(archived).not.toContain('1.3');
		expect(archived).not.toContain('1.4');
		expect(archived).not.toContain('1.5');

		// Verify deleted
		const task1Loaded = await loadEvidence(tempDir, '1.1');
		expect(task1Loaded.status).toBe('not_found');
		const task2Loaded = await loadEvidence(tempDir, '1.2');
		expect(task2Loaded.status).toBe('not_found');
		const task3Loaded = await loadEvidence(tempDir, '1.3');
		expect(task3Loaded.status).toBe('found');
		const task4Loaded = await loadEvidence(tempDir, '1.4');
		expect(task4Loaded.status).toBe('found');
		const task5Loaded = await loadEvidence(tempDir, '1.5');
	...
	test('archiveEvidence with both age and maxBundles: age first, then maxBundles', async () => {
		...
		const taskIds = ['1.1', '1.2', '1.3', '1.4', '1.5', '1.6'];
		for (const taskId of taskIds) {
			await saveEvidence(tempDir, taskId, createNoteEvidence(taskId, `Note for ${taskId}`));
		}
		...
		await makeBundleOld(tempDir, '1.1', 100);
		await makeBundleOld(tempDir, '1.2', 80);
		await makeBundleOld(tempDir, '1.3', 60);
		await makeBundleOld(tempDir, '1.4', 40);
		await makeBundleOld(tempDir, '1.5', 20);
		await makeBundleOld(tempDir, '1.6', 10);
		...
		expect(archived).toContain('1.1');
		expect(archived).toContain('1.2');
		expect(archived).toContain('1.3');
		expect(archived).toContain('1.4');
		expect(archived).not.toContain('1.5');
		expect(archived).not.toContain('1.6');

		// Verify deleted
		const task1Result = await loadEvidence(tempDir, '1.1');
		expect(task1Result.status).toBe('not_found');
		const task2Result = await loadEvidence(tempDir, '1.2');
		expect(task2Result.status).toBe('not_found');
		const task3Result = await loadEvidence(tempDir, '1.3');
		expect(task3Result.status).toBe('not_found');
		const task4Result = await loadEvidence(tempDir, '1.4');
		expect(task4Result.status).toBe('not_found');
		const task5Result = await loadEvidence(tempDir, '1.5');
		expect(task5Result.status).toBe('found');
		const task6Result = await loadEvidence(tempDir, '1.6');

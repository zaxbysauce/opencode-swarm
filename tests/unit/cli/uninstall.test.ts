...
describe('CLI uninstall command', () => {
    ...
    test('Uninstall with --clean removes config files', async () => {
        ...
        await writeFile(pluginConfigPath, JSON.stringify(pluginConfigData, null, 2));

        const promptsDir = join(tempDir, 'opencode', 'opencode-swarm');
        await mkdir(promptsDir, { recursive: true });
        await writeFile(join(promptsDir, 'architect.md'), '# Custom Architect Prompt');

        // Run: Run uninstall with --clean flag
        const result = await runCLI(['uninstall', '--clean'], { XDG_CONFIG_HOME: tempDir });

...
// ---------------------------------------------------------------------------
// macOS executor — import (placeholder throws on construction)
// ---------------------------------------------------------------------------
...
describe('MacOSSandboxExecutor', () => {
	...
	describe('constructor', () => {
		test('accepts scopePaths array when implemented (Phase 3)', () => {
			...
			expect(true).toBe(true); // Placeholder — remove when Phase 3 implements
		});
	...
	describe('getEnvOverrides()', () => {
		...
		test('returns empty object when no DYLD vars relevant (non-macOS)', () => {
			// On non-macOS, sandbox-exec doesn't exist — no DYLD vars to unset.
			if (isMac) return;
	...
	describe('_internals — DI seam (Phase 3)', () => {
...
// ---------------------------------------------------------------------------
// edge-cases — detectDyldInjection
...
describe('detectDyldInjection', () => {
	...
	test('returns false on non-macOS platforms', () => {
		if (isMac) return;
		// On non-macOS, DYLD_* vars don't exist — detection returns false.
...
// ---------------------------------------------------------------------------
// edge-cases — detectTmpDirManipulation
...
// ---------------------------------------------------------------------------
// edge-cases — detectSandboxProfileBypass
...
// ---------------------------------------------------------------------------
// edge-cases — detectSIPSProtectedPath
...
// ---------------------------------------------------------------------------
// edge-cases — detectEntitlementEscalation
...
// ---------------------------------------------------------------------------
// edge-cases — detectQuarantineBypass
...
// ---------------------------------------------------------------------------
// edge-cases — detectSandboxExecItself

...
// ---------------------------------------------------------------------------
// Error passthrough contract — applySandboxExecution fallback behavior
// ---------------------------------------------------------------------------
...
describe('applySandboxExecution — error passthrough contract', () => {
	...
	test('when executor.wrapCommand throws, original command is NOT modified', async () => {
		...
		// the command string — they fall through so the OS-level error
	...
	test('sandbox errors are distinguishable from OS-level errors', () => {
		...
		// The names differ — coders can distinguish the source
		expect(sandboxErr.name).toBe('SandboxError');
		expect(osErr.name).toBe('Error');

		// The codes differ — even if both had the same message, the code would differ
...
describe('resolveScopePaths — error and edge cases', () => {
	let tempDir: string;
	...
	describe('non-existent paths', () => {
		test('non-existent path returns warning but does NOT reject', () => {
			...
			// but is NOT rejected — this allows scope to include files to-be-created.
		...
		test('mixed existing and non-existing paths — non-existing gets warning but allowed', () => {
...
// ---------------------------------------------------------------------------
// Capability probe — binary-not-found error handling
// ---------------------------------------------------------------------------

describe('SandboxCapabilityProbe — binary-not-found handling', () => {
	...
	test('windows probe always returns enabled without error', async () => {
		...
		expect(result.mechanism).toBe('Restricted Token');
...
describe('FR-006 error distinguishability — full contract', () => {
	...
	test('sandbox error does not alter the command string when wrapping fails', () => {
		...
		try {
			...
			// Falls through — commandToExecute retains original value
		}

...
describe('resolveScopePaths', () => {
	...
	afterEach(() => {
		// Clean up temp directory — ignore errors on Windows where handles may linger
		try {
	...
	describe('relative path resolution', () => {
		...
		test('normalizes ./ prefix in relative paths', () => {
			...
			// ./lib/./lib normalizes to lib\lib on Windows, lib/lib on Unix —
	...
	describe('symlink traversal detection', () => {
		// Probe whether we can create symlinks — use a standalone temp dir so this
		...
		test.if(canCreateSymlink)(
			...
			() => {
				...
				// The symlink resolves to a path outside projectRoot — should be rejected
		...
		test.if(canCreateSymlink)(
			...
			() => {
				...
				// Should be allowed — resolves inside projectRoot.

...
describe('Prompt contract fixes', () => {
	it('coder supports explicit BLOCKED output alongside DONE output', () => {
		...
		expect(prompt).toContain('If the task is blocked, begin directly with BLOCKED.');
		expect(prompt).toContain('BLOCKED: [what went wrong]');
		expect(prompt).toContain('NEED: [what additional context or change would fix it]');
	});

	it('reviewer input format accepts diff and affected-surface context', () => {
		const prompt = createReviewerAgent('gpt-4').config.prompt ?? '';

		expect(prompt).toContain('FILE: [primary changed file or diff entry point]');
		expect(prompt).toContain('DIFF: [changed files/functions, or "infer from FILE" if omitted]');

...
describe('Task 6.2: Conditional adversarial-step changes', () => {
	...
	it('enabled=false omits step 5m and marks checklist SKIPPED', () => {
		...
		expect(prompt).toContain('test_engineer-adversarial: SKIPPED — disabled by config');
	});
	...
	it('scope=security-only makes step 5m conditional for security-sensitive work', () => {
		...
		// Checklist should show conditional PASS/FAIL/SKIP
		expect(prompt).toContain('test_engineer-adversarial: PASS / FAIL / SKIP — not security-sensitive');
	});

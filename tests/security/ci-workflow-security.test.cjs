...
class WorkflowSecurityTester {
  ...
  load() {
    try {
      ...
      console.error(`${colors.red}ERROR: Failed to load workflow file: ${error.message}${colors.reset}`);
      return false;
    }
  }
  ...
  testExpressionInjection() {
    ...
    function checkExpressions(obj, path = '') {
      if (typeof obj === 'string') {
        ...
        obj.forEach((item, index) => checkExpressions(item, `${path}[${index}]`));
      } else if (obj && typeof obj === 'object') {
        Object.keys(obj).forEach(key => checkExpressions(obj[key], `${path}.${key}`));
      }
    }
  ...
  testActionVersionPinning() {
    ...
    function findActions(obj, path = '') {
      if (obj && typeof obj === 'object') {
        ...
        Object.keys(obj).forEach(key => findActions(obj[key], `${path}.${key}`));
      }
    }
  ...
  testPermissions() {
    ...
    if (!this.workflow.permissions) {
      result.issues.push({
        ...
        severity: 'high',
        message: 'No permissions block declared. Default GITHUB_TOKEN gets write permissions which may be excessive.'
      });
    } else {
      result.passed = true;
      result.issues.push({
      ...
      if (this.workflow.permissions === 'write-all' || this.workflow.permissions === 'read-all') {
        result.passed = false;
        result.issues.push({
  ...
  testSecretExposure() {
    ...
    function checkSecrets(obj, path = '') {
      if (typeof obj === 'string') {
        ...
        const echoWithVars = /^(echo|print|Write-Host|console\.log)\s+.*\$\{/.test(obj);
        if (echoWithVars) {
          result.issues.push({
            location: path,
            severity: 'warning',
        ...
        Object.keys(obj).forEach(key => checkSecrets(obj[key], `${path}.${key}`));
      }
    }
  ...
  testCommandInjection() {
    ...
    const dangerousPatterns = [
      { pattern: /\$[a-zA-Z_][\w]*\s*;/, risk: 'Variable in command context without proper quoting' },
      { pattern: /`[^`]*`/, risk: 'Command substitution backticks' },
      { pattern: /\$\([^)]*\)/, risk: 'Command substitution $()' },
      { pattern: /\|\s*\w+/, risk: 'Pipe to potentially unsafe command' },
      { pattern: />\s*\$\(/, risk: 'Output redirection with command substitution' }
    ];
    ...
    function checkRunSteps(obj, path = '') {
      if (obj && typeof obj === 'object') {
        ...
        Object.keys(obj).forEach(key => checkRunSteps(obj[key], `${path}.${key}`));
      }
    }
  ...
  testTriggers() {
    ...
    if (this.workflow.on && this.workflow.on.workflow_run) {
      result.issues.push({
        severity: 'info',
  ...
  testResourceExfiltration() {
    ...
    function checkExfiltration(obj, path = '') {
      if (obj && typeof obj === 'object') {
        ...
        if (obj.uses && obj.uses.includes('upload-artifact')) {
          const pathVal = obj.with && obj.with.path;
          if (pathVal && (pathVal.includes('node_modules') || pathVal.includes('.git'))) {
            result.passed = false;
            result.issues.push({
              location: path,
        ...
        if (obj.run) {
          if (obj.run.includes('curl') || obj.run.includes('wget') || obj.run.includes('Invoke-WebRequest')) {
        ...
        Object.keys(obj).forEach(key => checkExfiltration(obj[key], `${path}.${key}`));
      }
    }
      ...
      if (result.issues && result.issues.length > 0) {
        for (const issue of result.issues) {
          if (issue.severity === 'info') {
            console.log(`  ${colors.cyan}ℹ INFO${colors.reset}: ${issue.message || issue.note || issue.risk}`);
            ...
            console.log(`  ${colors.red}⚠ ${issue.severity || 'FINDING'}${colors.reset}: ${issue.message || issue.risk}`);
            if (issue.location) console.log(`    Location: ${issue.location}`);
            if (issue.expression || issue.command) console.log(`    Pattern: ${issue.expression || issue.command}`);
          }
        }
      }
    }

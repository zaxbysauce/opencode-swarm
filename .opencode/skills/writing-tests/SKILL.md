...
6. **Fix path**: Migrate the affected test file to `_internals` DI seam pattern per the `mock-to-internals-migration` skill. This eliminates both the `vi.mock()` call and the closure capture surface area. **Note:** migration to `_internals` only helps when the source code actually calls `_internals.fn()` at the call site. If the source passes the function as a direct reference at definition time (see Tier 1 Critical Limitation below), `_internals` property replacement has the same closure capture problem — verify observable outcomes instead.

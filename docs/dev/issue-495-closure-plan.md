...
The system has **two parallel guard layers**, both confirmed during the second-pass validation:

- `src/hooks/scope-guard.ts` — added in commit `2dc8bf2` (v6.31 Task 3.1, **predates PR #501**). Hard-blocks `Edit`/`Write`/`Patch` from coder when the touched path is outside `declaredCoderScope` or `pendingCoderScopeByTaskId`. **Its own file header (lines 19–23) explicitly acknowledges the bash bypass as a "known architectural limitation"** and points at `diff-scope.ts` for "secondary coverage."
- `src/hooks/guardrails.ts` — `checkFileAuthorityWithRules` (line 3399) plus the `declaredCoderScope` wiring added in PR #501 (commit `e95bbce`). Uses prefix-rule evaluation; the v6.70.0 changes removed coder's hardcoded `allowedPrefix` so **any write is blocked unless declared scope grants it.**
- `src/hooks/diff-scope.ts` — post-hoc validator. Reads `.swarm/plan.json:phases[].tasks[].files_touched`, compares against `git diff --name-only`. **Never throws** (line 5 — "Returns a warning string … or null"). This is advisory, not a block. It catches bash bypasses *after* the fact and only when git diff is run.

| # | Hypothesis | Verdict | Evidence |
|---|-----------|---------|----------|
| A | Bash is outside the write-authority guard. `isWriteTool` gates the check; bash is not in `WRITE_TOOL_NAMES`. | **CONFIRMED (Critical)** — and explicitly acknowledged in source | `src/config/constants.ts:230-244` — comment lines 230-232 state "bash/shell tools are intentionally excluded — bash commands are opaque to static scope analysis." `WRITE_TOOL_NAMES` list at lines 234-244 contains `write, edit, patch, apply_patch, create_file, insert, replace, append, prepend` — no bash. `src/hooks/scope-guard.ts:19-23` repeats the same acknowledgment. `src/hooks/guardrails.ts:2021` (`if (isWriteTool(input.tool))`) is the only authority entry point. `checkDestructiveCommand` at `src/hooks/guardrails.ts:1042-1348` inspects bash for **deletion** patterns only — does not flag write redirects (`>`, `>>`), `sed -i`, `perl -i`, `cat > file`, `echo > file`, `printf > file`, `tee > file`. |
| B | `declare_scope` is in-memory only; does not persist to disk, does not cross process boundaries. | **CONFIRMED (High)** | `src/tools/declare-scope.ts:322` — `session.declaredCoderScope = mergedFiles` (session memory). Fallback `pendingCoderScopeByTaskId` at `src/hooks/delegation-gate.ts:45` is a module-level in-memory Map. Both `scope-guard.ts:79-83` and `resolveDeclaredScope` at `src/hooks/guardrails.ts:1515-1523` read only in-memory state. The user's #496 log line `cat .swarm/scope-1.1.json … no scope file` is a direct on-disk confirmation. |
...
| E | "Diff-scope provides secondary coverage" claim in `constants.ts:230` is misleading. | **CONFIRMED (Important framing)** | `src/hooks/diff-scope.ts:1-6` — "Never throws … Returns a warning string." It is advisory only and runs at git diff time, not at execution time. By the time it fires, the file mutation has already happened, the test has already run with mutated state, and any side effects (npm install, codegen) have already executed. It is detection, not prevention. |

**Refined root-cause story (post-validation):** PR #501 removed coder's hardcoded `allowedPrefix` for sound reasons (cross-framework compatibility) and wired `declaredCoderScope` into the authority check. But the wiring reads from in-memory state only, and the user-visible failure is **compound**: (1) architect doesn't reliably call `declare_scope` (no prompt rule), (2) when it does, the in-memory state doesn't survive cross-process delegation, so (3) coder's `Edit` is blocked with "not in allowed list," and (4) bash is unguarded so the coder achieves the write that way. Fixing only one layer leaves the user worse off than before (Edit denied + bash works = inconsistent UX). All four layers must be addressed.

**Note on `plan.json:files_touched` as a richer scope source:** `diff-scope.ts:15-47` already reads `.swarm/plan.json:phases[].tasks[].files_touched`. This is a richer, persistent source of intended scope than the in-memory `declaredCoderScope`. A cheaper alternative (or complement) to §2.3 #1 disk persistence would be to teach `scope-guard.ts` and `guardrails.ts` to **read `files_touched` from `plan.json` as a fallback** when `declaredCoderScope` is empty. Architect plan generation would no longer require an explicit `declare_scope` call — the plan itself becomes the scope source. See §2.3 #1b below.
...
1b. **(Alternative / complement) Use `plan.json:files_touched` as the durable scope source.**
   - `src/hooks/diff-scope.ts:15-47` already reads this. Teach `src/hooks/scope-guard.ts:79-83` and `src/hooks/guardrails.ts` `resolveDeclaredScope` (line 1515) to read `plan.json:phases[].tasks[].files_touched` for the active task as a fallback when `declaredCoderScope` is empty.
   - Pro: scope is already persistent (plan.json lives on disk and is the canonical task description). No new file format. No new race conditions.
   - Pro: architect cannot forget to declare scope — the plan IS the scope.
   - Pro: covers GPT 5.4's "declared ⊆ approved" invariant naturally — the only scope source is the plan, so divergence is impossible.
   - Con: requires that `currentTaskId` be reliably populated on the session before the coder runs. Verify against `swarmState.agentSessions` lifecycle.
   - Con: `files_touched` is the architect's intent, not necessarily what the coder needs (e.g., generated files, lockfiles). May need an opt-in `additional_files` field per task that the architect sets, or accept that coder hits a block and architect adds the file via a plan amendment.
   - **Recommendation: implement #1b in addition to #1.** Plan-as-scope is the durable answer; explicit declare_scope remains as a per-task override.
...
### 2.5b Out-of-scope second-pass observation: declared-vs-approved scope invariant

A second-opinion review (GPT 5.4) suggested the original #496 root cause was that `declaredCoderScope` was allowed to **diverge from the approved/report scope** without failing closed. After validating against the actual code and the actual #496 issue body:
- The validator-cited file (`src/hooks/scope-guard.ts`) and the bash-bypass acknowledgment in `constants.ts:230-232` are **real** and were not previously surfaced in the plan; both have now been added to §2.2.
- The validator's specific evidence quotes (`previousReportPaths: []`, `currentReportPaths: ["packages/tui/internal/client/client.go", …]`) **do not appear in the actual #496 issue body**, which is a Rails project (`/Users/lockstep/Personal/safai`) with files like `Gemfile`, `test/models/email_account_test.rb`, `config/environments/test.rb`. The TUI/opencode paths are likely conflated from the upstream `sst/opencode` codebase. Treat the divergence narrative as unverified.
- The validator's named functions (`extractDeclaredScopePaths`, `validateDeclaredScope`, `enforceScopeGuard`) **do not exist** in `scope-guard.ts`. Actual exports are `createScopeGuardHook`, `isFileInScope`. The validator likely confabulated function names by inference from the file's purpose.
- The validator's attribution of the scope-guard fix to PR #501 is **incorrect**. `scope-guard.ts` was added in commit `2dc8bf2` (v6.31, "feat(reliability): implement #401 reliability backlog"), well before PR #501 (`c884a33`). PR #501 added the cross-platform `allowedPrefix` removal + lstat + cwd containment.

**However, the underlying defensive idea is still worth tracking** as a separate concern: even with disk-persisted declared scope, the system should ensure that no scope source ever exceeds the architect-approved plan. If §2.3 #1b is implemented (plan.json as scope source), this invariant holds by construction. If §2.3 #1 is implemented standalone (disk-persisted declare_scope), add a cross-check that asserts declared scope is a subset of `plan.json:files_touched` for the active task.

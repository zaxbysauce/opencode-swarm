...
Tools receive execution context via `ToolContext`, enabling them to operate correctly regardless of the project directory structure.

### Working Directory Awareness

The `test_runner` tool demonstrates proper context threading:

```typescript
// Tool receives cwd from context
export const test_runner = tool({
  parameters: z.object({
    // ... parameters
    cwd: z.string().optional(), // Optional override
  }),
  execute: async (args, context: ToolContext) => {
    const cwd = args.cwd ?? context.cwd; // Use context cwd as default
    // All framework detection respects the project directory
...
Framework detection now respects the project directory via `cwd`:

| Framework | Detection Method | Cwd-Aware |
|-----------|-----------------|:---------:|
| Vitest | `vitest.config.*` presence in cwd | ✅ |
| Jest | `jest.config.*` or package.json `jest` field | ✅ |
| Mocha | `mocha` in package.json devDependencies | ✅ |
| pytest | `pytest` in requirements.txt or pyproject.toml | ✅ |
| Cargo | `Cargo.toml` in cwd | ✅ |
| Pester | `*.Tests.ps1` files in cwd | ✅ |

This enables accurate framework detection for projects at any directory level, not just the workspace root.

---

## Tool Filtering API Reference
...
Maps each agent to its allowed toolset:

```typescript
export const AGENT_TOOL_MAP: Record<AgentName, ToolName[]> = {
  architect: ['checkpoint', 'complexity_hotspots', 'detect_domains', ...], // 17 tools
  reviewer: ['diff', 'imports', 'lint', 'pkg_audit', ...], // 11 tools
  coder: ['diff', 'imports', 'lint', 'symbols', ...], // 6 tools
  // ... other agents
};
```

Runtime validation ensures all tool names in `AGENT_TOOL_MAP` exist in `TOOL_NAME_SET`.

### Types

#### `ToolName`
**Location**: `src/tools/tool-names.ts`
...
Used for runtime validation of `AGENT_TOOL_MAP` at startup.

#### `TOOL_NAMES`
**Location**: `src/tools/tool-names.ts`

Readonly array of all tool names for iteration:

```typescript
export const TOOL_NAMES: readonly ToolName[] = [
  'diff', 'lint', 'secretscan', 'test_runner', ...
];
```
...
Strips swarm prefixes from agent names to get canonical base names:

```typescript
export function stripKnownSwarmPrefix(agentName: string): string
```

Supports underscore, hyphen, and space separators. Case-insensitive.

```typescript
stripKnownSwarmPrefix('local_coder');      // → 'coder'
stripKnownSwarmPrefix('cloud-architect');  // → 'architect'
stripKnownSwarmPrefix('team alpha sme');   // → 'sme'
stripKnownSwarmPrefix('coder');            // → 'coder' (unchanged)
```

Used by tool filtering to apply overrides for prefixed agents (e.g., `local_coder` uses the `coder` override).
...
Tool filtering limits which tools each agent can access based on their role. This architectural constraint prevents agents from using tools inappropriate to their responsibilities, reducing errors and keeping agents focused.

### Design Rationale

| Without Filtering | With Filtering |
|-------------------|----------------|
| Coder might run security scans (wrong concern) | Coder focuses on writing code |
| Reviewer might edit files (bypasses QA) | Reviewer is read-only |
| All agents have 17+ tools in context | Agents have 3-11 relevant tools |
| Tool misuse wastes tokens | Focused toolsets improve efficiency |

### Implementation

Tool filtering is applied at agent configuration time in `getAgentConfigs()`:

```typescript
// From AGENT_TOOL_MAP in src/config/constants.ts
const allowedTools = AGENT_TOOL_MAP[agentName] ?? [];

// Filter the agent's tools to only allowed ones
sdkConfig.tools = filterTools(originalTools, allowedTools);
```

The `AGENT_TOOL_MAP` defines which tools each agent can use:
- **architect**: All 17 tools (orchestrator needs full visibility)
- **reviewer**: Security-focused set (11 tools)  
- **coder**: Write-focused minimal set (6 tools)
- **explorer**: Discovery-focused set (9 tools)
- **Other agents**: Role-appropriate subsets

### Runtime Validation

Tool names are validated at startup against `TOOL_NAME_SET` (from `src/tools/tool-names.ts`). Invalid tool names in `AGENT_TOOL_MAP` throw errors immediately.

### Configuration Overrides

Users can customize tool allocations via `tool_filter.overrides`:
...
Overrides are applied per-agent using `stripKnownSwarmPrefix()` to handle prefixed agent names (e.g., `local_coder` → `coder`).

### Tool Registry

The central tool registry (`src/tools/tool-names.ts`) provides:
- `ToolName`: Union type of all valid tool names
- `TOOL_NAMES`: Readonly array for iteration
- `TOOL_NAME_SET`: Set for O(1) validation

...
Triggered by `/swarm pr-review`. A read-only, structured review: intent reconstruction -> all 6 fixed base explorer lanes launched with `dispatch_lanes_async` while the architect continues non-dependent PR inspection -> incremental `collect_lane_results` polling and final join barrier -> every triggered micro-lane for risk categories present in the diff -> independent reviewer confirmation -> critic challenge on HIGH/CRITICAL -> synthesis only after coverage closure. The architect checks out the PR branch locally before exploring (explorers read the working tree, not git history). Does NOT mutate source or delegate to the coder. If lane tools cannot close coverage, the workflow falls back to blocking `dispatch_lanes`, then Task-tool dispatch as the final verified-equivalent fallback; if equivalence cannot be proven, the review is BLOCKED and surfaced to the user rather than degraded.
...
> **CODEBASE REALITY CHECK (v6.29.2):** Before any planning (MODE: PLAN), spec generation (MODE: SPECIFY), or plan import, the Architect verifies the current state of every referenced item. By default it partitions the referenced surface into disjoint scopes and dispatches them as asynchronous Explorer lanes (`dispatch_lanes_async`, lane count scaled to surface size up to the 8-lane cap), continuing non-dependent work while they run and joining at a hard settlement gate (`collect_lane_results` → `all_settled`) before any spec/plan/scope/implementation work proceeds. Produces a CODEBASE REALITY REPORT with statuses: NOT STARTED, PARTIALLY DONE, ALREADY COMPLETE, or ASSUMPTION INCORRECT. This prevents planning against stale assumptions (e.g., assuming a function exists when it was already fixed). Skipped for purely greenfield projects with no existing codebase references. See `.opencode/skills/pre-phase-briefing/SKILL.md` for the full protocol.

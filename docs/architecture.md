...
#### PR_REVIEW Protocol
Triggered by `/swarm pr-review`. A read-only, structured review: intent reconstruction -> 6 parallel explorer lanes launched with `dispatch_lanes_async` while the architect continues non-dependent PR inspection -> `collect_lane_results` join barrier -> independent reviewer confirmation -> critic challenge on HIGH/CRITICAL -> synthesis. The architect checks out the PR branch locally before exploring (explorers read the working tree, not git history) and launches the skill's triggered micro-lanes for risk categories present in the diff. Does NOT mutate source or delegate to the coder.
